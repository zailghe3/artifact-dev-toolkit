...
- `Repair package lock` is a maintainer-triggered trusted workflow. It checks out only current trusted `main`, regenerates and validates the lockfile there, discards validation side effects, bounds the file size, enforces a `package-lock.json`-only diff, and publishes a dedicated repair PR. PR-controlled application and dependency code never runs with its write token.
