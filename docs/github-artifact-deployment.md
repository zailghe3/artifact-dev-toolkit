...
Direct-write invalidation first advances a repository generation marker and then removes the pointer. Refreshes compare their starting generation again before pointer publication, so work started before a write cannot republish afterward; orphaned immutable chunks are harmless. Cache reads and writes that fail do not turn healthy GitHub reads or successful GitHub writes into failures: fresh GitHub content is returned as `degraded`, safe stable events are logged, and the UI distinguishes cache degradation from stale GitHub content. Concurrent isolates may idempotently write the same deterministic revision chunks; a writer that loses to KV contention accepts another already-complete snapshot for that revision. Manual refresh precedence is ordinary read, forced revision check, then full rebuild, with stronger work queued rather than downgraded.
