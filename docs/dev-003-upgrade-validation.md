...
- `eslint-config-next` 16 provides native flat-config exports. DEV-003 therefore migrates `eslint.config.mjs` from `FlatCompat` string extends to native `eslint-config-next/core-web-vitals` and `eslint-config-next/typescript` flat-config exports with `defineConfig` and `globalIgnores`, preserving the existing ignore patterns and Next.js core-web-vitals/TypeScript coverage.
