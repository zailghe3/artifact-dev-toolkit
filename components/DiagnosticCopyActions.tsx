...
import { diagnosticSummary, diagnosticTechnicalDetails, type SafeDiagnosticReport } from "@/lib/diagnostic-reports";
import { buttonStyles } from "./Ui";

export function DiagnosticCopyActions({ report }: { report: SafeDiagnosticReport }) {
  const [message, setMessage] = useState("");
  async function copy(kind: "summary" | "technical") {
    try { await navigator.clipboard.writeText(kind === "summary" ? diagnosticSummary(report) : diagnosticTechnicalDetails(report)); setMessage(`${kind === "summary" ? "Summary" : "Technical details"} copied.`); }
