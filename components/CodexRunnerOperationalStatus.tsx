...
export function CodexRunnerOperationalStatus({connection,expectedDigest}:{connection:SafeCodexConnectionStatus;expectedDigest?:string}){
 const[state,setState]=useState<OperationalLoadState>({status:"loading"}),[pending,setPending]=useState(false),inFlight=useRef(false),stateRef=useRef(state);stateRef.current=state;
 const refresh=useCallback(async()=>{if(inFlight.current)return;inFlight.current=true;setPending(true);try{const response=await fetch("/api/workflow-connections/codex-runner/jobs",{cache:"no-store"});if(!response.ok)throw new Error();const data=await response.json() as RunnerJobList;setState({status:"loaded",data,confirmedAt:new Date().toISOString()})}catch{const current=stateRef.current;setState({status:"unavailable",...(current.status==="loaded"?{previous:current.data,confirmedAt:current.confirmedAt}:current.status==="unavailable"&&current.previous?{previous:current.previous,confirmedAt:current.confirmedAt}:{})})}finally{inFlight.current=false;setPending(false)}},[]);
