...
import {EntityActions,EntityCard,buttonStyles} from "@/components/Ui";
...
export function DefinitionCatalogue({kind,items}:Props) {
 ...
 return <section className="grid gap-4"><ul className="grid gap-4">{items.map(item=>{const definition=item.definition,feedbackId=`delete-${definition.id}-feedback`;return <li key={definition.id}><EntityCard href={`/workflows/${kind==="agent"?"agents":"definitions"}/${definition.id}`} label={`Open ${kind} ${definition.name}`} actions={<div aria-label={`Actions for ${definition.name}`} className="grid w-full gap-2"><EntityActions danger={<button type="button" aria-label={`Delete ${kind} ${definition.name}`} aria-describedby={feedbackId} className={buttonStyles.danger} disabled={Boolean(pendingId)} aria-busy={pendingId===definition.id} onClick={()=>remove(item)}><PendingButtonContent pending={pendingId===definition.id}>{pendingId===definition.id?"Deleting…":"Delete"}</PendingButtonContent></button>}><Link className={buttonStyles.secondary} href={`/workflows/${kind==="agent"?"agents":"definitions"}/${definition.id}/edit`}>Edit</Link></EntityActions><ActionFeedback id={feedbackId} kind="error" message={errors[definition.id]}/></div>}><h2 className="text-lg font-bold">{definition.name}</h2><p className="mt-2 text-slate-600 dark:text-slate-300">{definition.description||"No description provided."}</p><p className="mt-3 text-sm">{"connectionKey" in definition?<>Connection: <code>{definition.connectionKey}</code></>:<>{definition.nodes.length} {definition.nodes.length===1?"block":"blocks"}</>}</p></EntityCard></li>})}</ul></section>;
}
