...
export function DefinitionCatalogue({kind,items}:Props) {
  const router=useRouter(),[error,setError]=useState(""),[pendingId,setPendingId]=useState<string>();
  async function remove(item:Versioned<AgentDefinitionV1|WorkflowDefinition>) {
    ...
    if(pendingId)return;setPendingId(item.definition.id);setError("");
    try{const endpoint=kind==="agent"?"workflow-agents":"workflow-definitions";
    ...
    router.refresh();
    }catch{setError("The definition could not be deleted.")}finally{setPendingId(undefined)}
  }
  return <section className="grid gap-4">{error&&<p role="alert" className="rounded border border-red-300 bg-red-50 p-3 text-red-800 dark:bg-red-950 dark:text-red-200">{error}</p>}<ul className="grid gap-4">{items.map(item=>{const definition=item.definition;return <li key={definition.id}><EntityCard href={`/workflows/${kind==="agent"?"agents":"definitions"}/${definition.id}`} label={`Open ${kind} ${definition.name}`} actions={<><Link className={workflowButton.secondary} href={`/workflows/${kind==="agent"?"agents":"definitions"}/${definition.id}/edit`}>Edit</Link><button type="button" aria-label={`Delete ${kind} ${definition.name}`} className={`${workflowButton.danger} ml-auto`} disabled={Boolean(pendingId)} aria-busy={pendingId===definition.id} onClick={()=>remove(item)}><PendingButtonContent pending={pendingId===definition.id}>{pendingId===definition.id?"Deleting…":"Delete"}</PendingButtonContent></button></>}><h2 className="text-lg font-bold">{definition.name}</h2><p className="mt-2 text-slate-600 dark:text-slate-300">{definition.description||"No description provided."}</p><p className="mt-3 text-sm">{"connectionKey" in definition?<>Connection: <code>{definition.connectionKey}</code></>:<>{definition.nodes.length} {definition.nodes.length===1?"block":"blocks"}</>}</p></EntityCard></li>})}</ul></section>;
}
