...
import type {AgentDefinitionV1,WorkflowDefinitionV1} from "@/lib/workflow-definitions";
import type {Versioned} from "@/lib/workflow-definition-repository";
import {EntityCard,workflowButton} from "@/components/WorkflowUi";

type Props = {kind:"agent";items:Versioned<AgentDefinitionV1>[]} | {kind:"workflow";items:Versioned<WorkflowDefinitionV1>[]};

export function DefinitionCatalogue({kind,items}:Props) {
  const router=useRouter(),[error,setError]=useState("");
  async function remove(item:Versioned<AgentDefinitionV1|WorkflowDefinitionV1>) {
