...
export function WorkflowLayoutEditor({workflow,agents,initialLayout}:{workflow:WorkflowDefinition;agents:Record<string,string>;initialLayout?:SavedLayout}){
 ...
 const [nodes,setNodes]=useState<Node[]>(()=>projected.nodes.map(node=>({...node,data:{label:<><strong>{node.data.kind==="step"?`Step ${node.data.stepNumber}: ${node.data.label}`:"Agent block"}</strong><span className="block text-xs">Agent: {agents[node.data.agentId]??node.data.agentId}</span></>}})));
 ...
 return <section className="mt-6" aria-labelledby="workflow-layout-heading">
  <div className="flex flex-wrap items-end justify-between gap-3"><div><h2 id="workflow-layout-heading" className="text-xl font-bold">Visual layout</h2><p className="text-sm text-slate-600 dark:text-slate-300">Drag nodes or move the viewport to arrange this view. Semantic edges determine the execution chain; layout changes never change what runs.</p></div><button type="button" onClick={save} className="rounded bg-sky-700 px-4 py-2 font-bold text-white">Save layout</button></div>
