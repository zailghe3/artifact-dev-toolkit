...
export async function POST(request:Request,{params}:{params:Promise<{connectionKey:string}>}){
 ...
 const {connectionKey}=await params;if(connectionKey!=="openai-primary")return NextResponse.json({ok:false,category:"connection_unavailable",message:"Connection is not configured."},{status:404,headers:noStoreHeaders});
 // Deliberately do not read the request body: every diagnostic input is server controlled.
 const result=await testWorkflowConnection(connectionKey,await getWorkflowProviderConnectionStore(),getWorkflowAdapterRegistry());
