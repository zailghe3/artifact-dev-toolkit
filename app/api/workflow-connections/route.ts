...
export async function POST(request:Request){const auth=await requireApiRepositoryAccess(request);if(auth instanceof Response)return auth;try{if(!sameOrigin(request))return NextResponse.json({code:"invalid_origin",error:"Request origin is invalid."},{status:403,headers:noStoreHeaders});const body=input.parse(await readBoundedJson(request,10240)),type=requireProviderConnectionType(body.adapter),store=await getWorkflowProviderConnectionStore(auth.access),vault=await getProviderCredentialVault(),repository=createWorkflowConnectionDefinitionRepository(auth.access);if(type.id==="work-iq-rest"){if(body.credential||!body.clientSecret)throw new Error("credential_required");await createDelegatedVaultConnection({connectionKey:body.connectionKey,displayName:body.displayName,adapter:body.adapter,configuration:body.configuration,clientSecret:body.clientSecret},store,vault,repository)}else await createConnectionForAuthentication(type,body.credential,async credential=>{await createApiKeyVaultConnection({...body,credential},store,vault,repository,(value,model,configuration)=>providerModelService.validate(body.adapter,value,model,configuration));return store});return NextResponse.json({connection:await store.getSafeDescriptor(body.connectionKey)},{status:201,headers:noStoreHeaders});}catch(error){return workflowError(error);}}
