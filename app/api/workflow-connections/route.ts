...
import {PROVIDER_CONNECTION_TYPE_IDS} from "@/lib/provider-connection-types";
...
const key=/^[a-z0-9]+(?:-[a-z0-9]+)*$/,input=z.object({connectionKey:z.string().max(80).regex(key),displayName:z.string().trim().min(1).max(120),adapter:z.enum(PROVIDER_CONNECTION_TYPE_IDS),model:z.string().trim().min(1).max(120).optional(),credential:z.string().min(1).max(8192)}).strict();
...
export async function POST(request:Request){const auth=await requireApiRepositoryAccess(request);if(auth instanceof Response)return auth;try{if(!sameOrigin(request))return NextResponse.json({code:"invalid_origin",error:"Request origin is invalid."},{status:403,headers:noStoreHeaders});const body=input.parse(await readBoundedJson(request,10240)),store=await getWorkflowProviderConnectionStore(auth.access),vault=await getProviderCredentialVault(),repository=createWorkflowConnectionDefinitionRepository(auth.access);await createTargetVaultConnection(body,store,vault,repository,(credential,model)=>providerModelService.validate(body.adapter,credential,model));return NextResponse.json({connection:await store.getSafeDescriptor(body.connectionKey)},{status:201,headers:noStoreHeaders});}catch(error){return workflowError(error);}}
