...
import {generateProviderCredentialVaultSecretId} from "@/lib/provider-credential-vault";
...
/** Git is written first. Any uncertain Git result is never retried or rolled back; a missing vault value is safely recoverable. */
export async function POST(request:Request){const auth=await requireApiRepositoryAccess(request);if(auth instanceof Response)return auth;try{if(!sameOrigin(request))return NextResponse.json({code:"invalid_origin",error:"Request origin is invalid."},{status:403,headers:noStoreHeaders});const body=input.parse(await readBoundedJson(request,10240));await validateOpenAIModel(body.credential,body.model);const secretRef=generateProviderCredentialVaultSecretId(),definition=connectionDefinitionSchema.parse({schemaVersion:1,id:body.connectionKey,name:body.displayName,runtime:body.adapter,provider:"openai",model:body.model,credential:{source:"adt-vault",secretRef}}),repository=createWorkflowConnectionDefinitionRepository(auth.access);if(!repository.createConnection)throw new Error("connection_repository_unavailable");await repository.createConnection(definition);try{await (await getProviderCredentialVault()).recover(secretRef,body.credential)}catch{throw new Error("vault_persistence_failed")}const connection=await (await getWorkflowProviderConnectionStore(auth.access)).getSafeDescriptor(body.connectionKey);return NextResponse.json({connection},{status:201,headers:noStoreHeaders});}catch(error){return workflowError(error);}}
