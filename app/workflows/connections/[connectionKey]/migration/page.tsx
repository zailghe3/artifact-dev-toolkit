...
export default async function Page({params}:{params:Promise<{connectionKey:string}>}){const {access}=await requireRepositoryAccess("/workflows/connections"),key=(await params).connectionKey;let migration;try{migration=await(await getWorkflowProviderConnectionMigrationService(access)).inspect(key)}catch(error){if(error instanceof ConnectionMigrationError&&error.code==="migration_source_not_found")notFound();throw error}return <><h1 className="text-3xl font-black">Prepare Git migration</h1><p className="mt-3 text-slate-600 dark:text-slate-300">Export safe D1 settings, provision the credential externally, then verify and explicitly retire the matching fallback.</p><ProviderConnectionMigration initial={migration}/></>}
