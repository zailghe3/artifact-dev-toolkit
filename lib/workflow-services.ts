...
export async function resolveManagedCodeRepository(environmentKey:string){const {getCodexRunnerClient}=await import("./codex-runner-client.ts"),client=getCodexRunnerClient(),descriptor=await client.managedRepository(environmentKey);return resolveManagedCodeRepositoryContext(descriptor,getGitHubAppIdentityConfig())}
export async function listWorkflowConnectionDescriptors(snapshot:CodexRunnerSnapshot|undefined,access:RepositoryAccessContext){const [catalog,providers]=await Promise.all([snapshot??readCodexRunnerCatalog(),getWorkflowProviderConnectionStore(access).then(store=>store.listSafeDescriptors())]);return assertUniqueConnectionDescriptors([...listConnectionDescriptors().map(item=>item.key==="codex-primary"?codexRunnerDescriptor(catalog.available):item),...providers]);}
/** Durable connection identities for definition authoring. This never contacts Codex Runner. */
export async function listWorkflowConnectionIdentityDescriptors(access:RepositoryAccessContext){const providers=await getWorkflowProviderConnectionStore(access).then(store=>store.listSafeDescriptors());return assertUniqueConnectionDescriptors([...listConnectionDescriptors(),...providers]);}
