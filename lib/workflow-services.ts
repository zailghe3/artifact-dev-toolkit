...
export async function listWorkflowConnectionDescriptors(snapshot?:CodexRunnerSnapshot,access?:RepositoryAccessContext){const [catalog,providers]=await Promise.all([snapshot??readCodexRunnerCatalog(),getWorkflowProviderConnectionStore(access).then(store=>store.listSafeDescriptors())]);return[...listConnectionDescriptors().map(item=>item.key==="codex-primary"?codexRunnerDescriptor(catalog.available):item),...providers];}
