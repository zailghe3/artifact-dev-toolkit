...
import { ArtifactProductionDeleteRequiresProposalError, ArtifactWriteConflictError, type ArtifactDeleteResult, type ArtifactProposalResult, type ArtifactWithRevision, type DeleteArtifactInput } from "./artifact-repository.ts";
...
export type DeletionProposalDependencies = Common & { propose(access: RepositoryAccessContext, input: DeleteArtifactInput): Promise<ArtifactProposalResult> };
...
export async function handleDirectDeletion(request: Request, id: string, dependencies: DirectDeletionDependencies) {
  ...
  try { const stored = await dependencies.load(parsed.authorization.access, id); if (!stored) return json({ error: "Artifact not found", code: "artifact_not_found" }, 404); if (stored.currentFileSha !== parsed.sha) throw new ArtifactWriteConflictError(); if (stored.artifact.status === "production") throw new ArtifactProductionDeleteRequiresProposalError(); return json(await dependencies.remove(parsed.authorization.access, { id, currentFileSha: parsed.sha, actorLogin: parsed.authorization.session.login }), 200); } catch (error) { return artifactWriteErrorResponse(error); }
}
export async function handleDeletionProposal(request: Request, id: string, dependencies: DeletionProposalDependencies) {
  const parsed = await authorizeAndParse(request, dependencies.authorize); if (parsed instanceof Response) return parsed;
  try { const stored = await dependencies.load(parsed.authorization.access, id); if (!stored) return json({ error: "Artifact not found", code: "artifact_not_found" }, 404); if (stored.currentFileSha !== parsed.sha) throw new ArtifactWriteConflictError(); if (stored.artifact.status !== "production") return json({ error: "Deletion proposals require a production artifact", code: "proposal_requires_production_artifact" }, 400); return json(await dependencies.propose(parsed.authorization.access, { id, currentFileSha: parsed.sha, actorLogin: parsed.authorization.session.login }), 201); } catch (error) { return artifactWriteErrorResponse(error); }
}
