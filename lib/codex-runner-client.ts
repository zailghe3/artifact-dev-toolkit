...
export class CodexRunnerError extends Error{constructor(public readonly category:RunnerErrorCategory){super(category)}}
...
export class CodexRunnerClient{
 ...
 private async request(path:string,method="GET"){const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),this.configuration.timeoutMs??8_000);let response:Response;try{response=await this.fetcher(new URL(path,this.baseUrl),{method,headers:{accept:"application/json","content-type":"application/json","CF-Access-Client-Id":this.configuration.accessClientId,"CF-Access-Client-Secret":this.configuration.accessClientSecret,"X-Codex-Runner-Secret":this.configuration.sharedSecret},signal:controller.signal,redirect:"error"});}catch{throw new CodexRunnerError("runner_unavailable")}finally{clearTimeout(timer)}
  if(response.status===401||response.status===403)throw new CodexRunnerError(response.headers.get("cf-ray")?"access_denied":"runner_unauthorized");if(!response.ok)throw new CodexRunnerError("runner_unavailable");const length=Number(response.headers.get("content-length")??0);if(length>MAX_RESPONSE_BYTES)throw new CodexRunnerError("invalid_response");const text=await response.text();if(Buffer.byteLength(text)>MAX_RESPONSE_BYTES)throw new CodexRunnerError("invalid_response");try{return JSON.parse(text) as unknown}catch{throw new CodexRunnerError("invalid_response")}}
