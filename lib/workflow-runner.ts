...
export class WorkflowRunner {
  ...
  private async cancellation(detail:RunDetail) { if(detail.run.status!=="cancelling"&&!detail.run.cancelRequestedAt)return false; const current=detail.attempts.filter(a=>a.stepId===detail.run.currentStepId).at(-1); let result:"cancelled"|"already_terminal"|"unsupported"|"safe_failure"="already_terminal";if(current?.providerTaskId){try{const invocation=this.invocation(detail,current);const adapter=this.adapters.get(invocation.connection.adapter);result=adapter?.cancel?await adapter.cancel(current.providerTaskId,invocation):"unsupported";}catch{result="safe_failure";}}await this.storage.recordCancellationResult(detail.run.id,result);await this.storage.cancelRun(detail.run.id); return true; }
  private invocation(detail:RunDetail,attempt:RunDetail["attempts"][number]):AdapterInvocation { const agent=detail.run.agentSnapshots[attempt.agentId]; return {runId:detail.run.id,stepId:attempt.stepId,iteration:attempt.iteration,attempt:attempt.attempt,providerPollCount:attempt.providerPollCount,idempotencyKey:`${detail.run.id}:${attempt.stepId}:${attempt.iteration}:${attempt.attempt}`,agentName:agent.name,masterPrompt:agent.masterPrompt,inputText:attempt.inputText??"",connection:this.resolve(attempt.connectionKey),providerOptions:agent.adapterOptions}; }
  async execute(runId:string):Promise<RunDetail> {
    for(;;){ let detail=await this.storage.getRun(runId); if(!detail)throw new Error("run_not_found"); if(["succeeded","failed","cancelled"].includes(detail.run.status))return detail; if(await this.cancellation(detail))return (await this.storage.getRun(runId))!;
      ...
      if(result.state==="pending"){await this.storage.incrementPollCount(runId,step.id,attempt.iteration,attempt.attempt);await this.boundary("wait-provider",async()=>{});continue;}
