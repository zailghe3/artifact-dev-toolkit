...
async function safeConfiguration() {
  ...
  return { backend: publicConfig.backend, ...(publicConfig.owner ? { owner: publicConfig.owner } : {}), ...(publicConfig.repository ? { repository: publicConfig.repository } : {}), branch: publicConfig.branch, artifactRoot: publicConfig.root, cacheBinding: presence("ARTIFACT_CATALOGUE_CACHE"), authSecrets };
}
