...
function validateGraphTopology(workflow:{nodes:Array<{id:string;blockType:string;blockVersion:number;config:unknown}>;edges:Array<{source:string;sourcePort?:string;target:string;targetPort?:string}>},context:z.RefinementCtx){
 ...
 for(const node of workflow.nodes){if(node.blockType!=="agent"||node.blockVersion!==2)continue;const selected=(node.config as {context?:{includeNodeOutputs?:string[]}}).context?.includeNodeOutputs??[];for(const sourceId of selected){const source=byId.get(sourceId),sourceContext=source?.blockType==="agent"&&source.blockVersion===2?(source.config as {context?:{publishOutput?:boolean}}).context:undefined;if(sourceId===node.id||!source||source.blockType!=="agent"||source.blockVersion!==2||sourceContext?.publishOutput!==true||!canReach(sourceId,node.id))context.addIssue({code:"custom",message:"Selected context must reference a publishing Agent block that can execute before this Agent.",path:["nodes",workflow.nodes.indexOf(node),"config","context","includeNodeOutputs"]});}}
...
export function compileWorkflowV2ExecutionPlan(workflow:WorkflowDefinitionV2,agents:readonly AgentDefinitionV1[]):GenericWorkflowExecutionPlan{
 const parsed=workflowDefinitionV2Schema.parse(workflow),agentIds=new Set(agents.map(agent=>agent.id)),byId=new Map(parsed.nodes.map(node=>[node.id,node])),incoming=new Map(parsed.nodes.map(node=>[node.id,0])),outgoing=new Map(parsed.nodes.map(node=>[node.id,0]));for(const edge of parsed.edges){incoming.set(edge.target,incoming.get(edge.target)!+1);outgoing.set(edge.source,outgoing.get(edge.source)!+1)}for(const node of parsed.nodes)for(const agentId of workflowBlockRegistry.references(node.blockType,node.blockVersion,node.config).agentIds??[])if(!agentIds.has(agentId))throw new Error(`missing_agent:${agentId}`);return genericWorkflowExecutionPlanSchema.parse({planVersion:2,nodes:parsed.nodes.map(node=>{const config=structuredClone(workflowBlockRegistry.validate(node.blockType,node.blockVersion,node.config));if(node.blockType==="agent"&&node.blockVersion===2){const context=(config as {context?:{publishOutput?:boolean;includeRunInput?:boolean;includeNodeOutputs?:string[]}}).context;if(!context?.publishOutput&&!context?.includeRunInput&&!context?.includeNodeOutputs?.length)return{id:node.id,blockType:"agent" as const,blockVersion:1 as const,config:{agentId:(config as {agentId:string}).agentId}}}return{id:node.id,blockType:node.blockType,blockVersion:node.blockVersion,config}}),edges:parsed.edges.map(edge=>{const source=byId.get(edge.source)!,target=byId.get(edge.target)!;return{source:edge.source,sourcePort:workflowBlockRegistry.port(source.blockType,source.blockVersion,"outputs",edge.sourcePort),target:edge.target,targetPort:workflowBlockRegistry.port(target.blockType,target.blockVersion,"inputs",edge.targetPort)}}),entryNodeId:graphEntryNode(parsed.nodes,parsed.edges),terminalNodeId:[...outgoing].find(([,count])=>count===0)![0],maxStepExecutions:parsed.limits.maxStepExecutions});
}
