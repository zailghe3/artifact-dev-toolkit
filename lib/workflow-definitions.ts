...
export const WORKFLOW_RUN_INPUT_MAX_LENGTH = 65536;
...
function validateGraphTopology(workflow:{nodes:Array<{id:string;blockType:string;blockVersion:number;config:unknown}>;edges:Array<{source:string;sourcePort?:string;target:string;targetPort?:string}>},context:z.RefinementCtx){
 ...
 for(const node of workflow.nodes)if(node.blockType==="publish-github-pr"){const source=(node.config as {sourceNodeId?:string}).sourceNodeId,ins=incoming.get(node.id)??[];if(!source||byId.get(source)?.blockType!=="agent"||ins.length!==1||ins[0]?.source!==source)context.addIssue({code:"custom",message:"Publish GitHub PR must directly follow its configured Codex Agent source.",path:["nodes",workflow.nodes.indexOf(node)]});}
 const canReach=(source:string,target:string)=>{const seen=new Set<string>();const visit=(current:string):boolean=>current===target?true:seen.has(current)?false:(seen.add(current),(outgoing.get(current)??[]).some(edge=>visit(edge.target)));return visit(source)};
 for(const node of workflow.nodes){if(node.blockType!=="agent"||![2,3].includes(node.blockVersion))continue;const selected=(node.config as {context?:{includeNodeOutputs?:string[]}}).context?.includeNodeOutputs??[];for(const sourceId of selected){const source=byId.get(sourceId),sourceContext=source?.blockType==="agent"&&[2,3].includes(source.blockVersion)?(source.config as {context?:{publishOutput?:boolean}}).context:undefined;if(sourceId===node.id||!source||source.blockType!=="agent"||![2,3].includes(source.blockVersion)||sourceContext?.publishOutput!==true||!canReach(sourceId,node.id))context.addIssue({code:"custom",message:"Selected context must reference a publishing Agent block that can execute before this Agent.",path:["nodes",workflow.nodes.indexOf(node),"config","context","includeNodeOutputs"]});}}
 const conversationNodes=workflow.nodes.filter(node=>node.blockType==="agent"&&node.blockVersion===3&&(node.config as {conversation?:unknown}).conversation);const parent=new Map(conversationNodes.map(node=>[node.id,(node.config as {conversation:{sourceNodeId:string}}).conversation.sourceNodeId]));
 for(const node of conversationNodes){const sourceId=parent.get(node.id)!,source=byId.get(sourceId);if(sourceId===node.id||!source||source.blockType!=="agent"||!canReach(sourceId,node.id))context.addIssue({code:"custom",message:"Conversation source must reference an earlier Agent block.",path:["nodes",workflow.nodes.indexOf(node),"config","conversation","sourceNodeId"]});const seen=new Set([node.id]);let cursor:string|undefined=sourceId;while(cursor&&parent.has(cursor)){if(seen.has(cursor)){context.addIssue({code:"custom",message:"Conversation references must not contain a cycle.",path:["nodes",workflow.nodes.indexOf(node),"config","conversation"]});break}seen.add(cursor);cursor=parent.get(cursor)}}
 const conversationRoot=(nodeId:string)=>{const seen=new Set<string>();let value=nodeId;while(parent.has(value)&&!seen.has(value)){seen.add(value);value=parent.get(value)!}return value};for(let i=0;i<conversationNodes.length;i++)for(let j=i+1;j<conversationNodes.length;j++){const a=conversationNodes[i].id,b=conversationNodes[j].id;if(conversationRoot(a)===conversationRoot(b)&&!canReach(a,b)&&!canReach(b,a))context.addIssue({code:"custom",message:"Conversation continuations in one lineage must be structurally ordered.",path:["nodes",workflow.nodes.indexOf(conversationNodes[j]),"config","conversation"]});}
 if(terminals.length===1){const terminalType=byId.get(terminals[0])?.blockType;if(terminalType!=="agent"&&terminalType!=="subworkflow"&&terminalType!=="publish-github-pr")context.addIssue({code:"custom",message:"The successful terminal must be an Agent, reusable Workflow, or Publish GitHub PR block.",path:["nodes"]});}
...
export const workflowDefinitionV2Schema=z.object({schemaVersion:z.literal(2),id,name:z.string().trim().min(1).max(120),description:z.string().max(2000),status:z.literal("draft"),runInputTemplate:z.string().max(WORKFLOW_RUN_INPUT_MAX_LENGTH).optional(),exposableAsBlock:z.boolean().optional(),nodes:z.array(workflowNodeSchema).min(1).max(MAX_WORKFLOW_STEPS),edges:z.array(workflowEdgeSchema).max(MAX_STEP_EXECUTIONS),limits:z.object({maxStepExecutions:z.number().int().min(1).max(MAX_STEP_EXECUTIONS)}).strict()}).strict().superRefine((workflow,context)=>{const nodeIds=new Set<string>(),edgeIds=new Set<string>();workflow.nodes.forEach((node,index)=>{if(nodeIds.has(node.id))context.addIssue({code:"custom",message:"Node IDs must be unique.",path:["nodes",index,"id"]});nodeIds.add(node.id);try{workflowBlockRegistry.validate(node.blockType,node.blockVersion,node.config)}catch(error){context.addIssue({code:"custom",message:error instanceof Error?error.message:"Invalid block configuration.",path:["nodes",index,"config"]})}});workflow.edges.forEach((edge,index)=>{if(edgeIds.has(edge.id))context.addIssue({code:"custom",message:"Edge IDs must be unique.",path:["edges",index,"id"]});edgeIds.add(edge.id)});validateGraphTopology(workflow,context)});
...
const genericPlanNode=z.union([
 z.object({id,blockType:z.literal("agent"),blockVersion:z.literal(1),config:z.object({agentId:id}).strict()}).strict(),
 z.object({id,blockType:z.literal("agent"),blockVersion:z.literal(2),config:z.object({agentId:id,context:z.object({publishOutput:z.boolean(),includeRunInput:z.boolean(),includeNodeOutputs:z.array(id).max(16)}).strict().optional()}).strict()}).strict(),
 z.object({id,blockType:z.literal("agent"),blockVersion:z.literal(3),config:z.object({agentId:id,context:z.object({publishOutput:z.boolean(),includeRunInput:z.boolean(),includeNodeOutputs:z.array(id).max(16)}).strict().optional(),conversation:z.object({sourceNodeId:id}).strict().optional()}).strict()}).strict(),
 ...
 z.object({id,blockType:z.literal("approval"),blockVersion:z.literal(1),config:z.object({message:z.string().trim().min(1).max(2000)}).strict()}).strict(), z.object({id,blockType:z.literal("publish-github-pr"),blockVersion:z.literal(1),config:z.object({sourceNodeId:id,title:z.string().trim().min(1).max(256),body:z.string().max(65536),draft:z.boolean()}).strict()}).strict(),
]);
...
export function composeVersionedWorkflowV2(root:VersionedWorkflowDefinition,versions:readonly VersionedWorkflowDefinition[]):ComposedWorkflowV2{
 ...
 const expand=(workflow:WorkflowDefinitionV2,path:Array<{workflowId:string;workflowName:string;invocationNodeId:string}>,stack:string[]):WorkflowDefinitionV2=>{if(stack.length>MAX_COMPOSITE_DEPTH)throw new Error("subworkflow_depth_exceeded");const replacements=new Map<string,{entry:string;terminal:string}>(),nodes:WorkflowDefinitionV2["nodes"]=[],edges:WorkflowDefinitionV2["edges"]=[];
  const executionId=(semanticNodeId:string)=>{const key=pathKey([...path.flatMap(item=>[item.workflowId,item.invocationNodeId]),workflow.id,semanticNodeId]);return path.length?scopedId("n",[key]):semanticNodeId};
  for(const node of workflow.nodes){const refs=workflowBlockRegistry.references(node.blockType,node.blockVersion,node.config).workflowIds??[];if(!refs.length){const key=pathKey([...path.flatMap(item=>[item.workflowId,item.invocationNodeId]),workflow.id,node.id]),executionNodeId=claim("n",executionId(node.id),key),config=node.blockType==="agent"&&[2,3].includes(node.blockVersion)?(()=>{const value=structuredClone(node.config) as {context?:{includeNodeOutputs?:string[]};conversation?:{sourceNodeId:string}};if(value.context?.includeNodeOutputs)value.context.includeNodeOutputs=value.context.includeNodeOutputs.map(executionId);if(value.conversation)value.conversation.sourceNodeId=executionId(value.conversation.sourceNodeId);return value})():node.config;nodes.push({...node,id:executionNodeId,config});const rootNodeId=path[0]?.invocationNodeId??node.id,labels=[rootWorkflow.name,...path.map(item=>item.workflowName),node.blockType==="agent"?node.id:workflowBlockRegistry.resolve(node.blockType,node.blockVersion).ui.label];provenance.push({executionNodeId,rootNodeId,workflowId:workflow.id,workflowName:workflow.name,semanticNodeId:node.id,invocationPath:path,displayPath:labels.join(" / ")});continue}const childId=refs[0];if(stack.includes(childId))throw new Error(`subworkflow_cycle:${[...stack,childId].join("->")}`);const versioned=available.get(childId);if(!versioned)throw new Error(`missing_subworkflow:${childId}`);if(versioned.definition.schemaVersion!==2)throw new Error(`subworkflow_not_v2:${childId}`);const child=versioned.definition;if(!child.exposableAsBlock)throw new Error(`subworkflow_not_exposed:${childId}`);used.set(childId,{workflowId:childId,revision:versioned.fileSha,definition:child,dependencyWorkflowIds:workflowSubworkflowReferences(child)});const expanded=expand(child,[...path,{workflowId:child.id,workflowName:child.name,invocationNodeId:node.id}],[...stack,childId]);const entry=graphEntryNode(expanded.nodes,expanded.edges),outgoing=new Set(expanded.edges.map(edge=>edge.source)),terminal=expanded.nodes.find(value=>!outgoing.has(value.id))!.id;replacements.set(node.id,{entry,terminal});nodes.push(...expanded.nodes);edges.push(...expanded.edges)}
...
export function compileWorkflowV2ExecutionPlan(workflow:WorkflowDefinitionV2,agents:readonly AgentDefinitionV1[]):GenericWorkflowExecutionPlan{
 const parsed=workflowDefinitionV2Schema.parse(workflow),agentIds=new Set(agents.map(agent=>agent.id)),byId=new Map(parsed.nodes.map(node=>[node.id,node])),incoming=new Map(parsed.nodes.map(node=>[node.id,0])),outgoing=new Map(parsed.nodes.map(node=>[node.id,0]));for(const edge of parsed.edges){incoming.set(edge.target,incoming.get(edge.target)!+1);outgoing.set(edge.source,outgoing.get(edge.source)!+1)}for(const node of parsed.nodes)for(const agentId of workflowBlockRegistry.references(node.blockType,node.blockVersion,node.config).agentIds??[])if(!agentIds.has(agentId))throw new Error(`missing_agent:${agentId}`);return genericWorkflowExecutionPlanSchema.parse({planVersion:2,nodes:parsed.nodes.map(node=>{const config=structuredClone(workflowBlockRegistry.validate(node.blockType,node.blockVersion,node.config));if(node.blockType==="agent"&&[2,3].includes(node.blockVersion)){const value=config as {agentId:string;context?:{publishOutput?:boolean;includeRunInput?:boolean;includeNodeOutputs?:string[]};conversation?:{sourceNodeId:string}},activeContext=Boolean(value.context?.publishOutput||value.context?.includeRunInput||value.context?.includeNodeOutputs?.length);if(node.blockVersion===3&&value.conversation)return{id:node.id,blockType:"agent" as const,blockVersion:3 as const,config:value};if(activeContext)return{id:node.id,blockType:"agent" as const,blockVersion:2 as const,config:{agentId:value.agentId,context:value.context}};return{id:node.id,blockType:"agent" as const,blockVersion:1 as const,config:{agentId:value.agentId}}}return{id:node.id,blockType:node.blockType,blockVersion:node.blockVersion,config}}),edges:parsed.edges.map(edge=>{const source=byId.get(edge.source)!,target=byId.get(edge.target)!;return{source:edge.source,sourcePort:workflowBlockRegistry.port(source.blockType,source.blockVersion,"outputs",edge.sourcePort),target:edge.target,targetPort:workflowBlockRegistry.port(target.blockType,target.blockVersion,"inputs",edge.targetPort)}}),entryNodeId:graphEntryNode(parsed.nodes,parsed.edges),terminalNodeId:[...outgoing].find(([,count])=>count===0)![0],maxStepExecutions:parsed.limits.maxStepExecutions});
}

/** Removes Worker-only conversation metadata before a plan crosses the ADT Runtime boundary. */
export function projectExecutionPlanForRuntime(plan:GenericWorkflowExecutionPlan):GenericWorkflowExecutionPlan{return genericWorkflowExecutionPlanSchema.parse({...plan,nodes:plan.nodes.map(node=>{if(node.blockType!=="agent"||node.blockVersion!==3)return structuredClone(node);const value=node.config,context=value.context,active=Boolean(context?.publishOutput||context?.includeRunInput||context?.includeNodeOutputs.length);return active?{id:node.id,blockType:"agent" as const,blockVersion:2 as const,config:{agentId:value.agentId,context}}:{id:node.id,blockType:"agent" as const,blockVersion:1 as const,config:{agentId:value.agentId}}})});}
...
export function validatePublishGitHubPrAuthoringSources(workflow:WorkflowDefinition,agents:readonly AgentDefinitionV1[],connections:readonly {key:string;adapter:string}[]){
 ...
 for(const node of workflow.nodes){if(node.blockType!=="agent"||node.blockVersion!==3)continue;const config=node.config as {agentId:string;conversation?:{sourceNodeId:string}};if(!config.conversation)continue;const source=workflow.nodes.find(value=>value.id===config.conversation!.sourceNodeId),targetAgent=agentsById.get(config.agentId),sourceAgent=source?.blockType==="agent"?agentsById.get((source.config as {agentId:string}).agentId):undefined;if(!targetAgent||!sourceAgent)throw new Error("conversation_source_invalid");const targetConnection=connectionsByKey.get(targetAgent.connectionKey),sourceConnection=connectionsByKey.get(sourceAgent.connectionKey),targetOptions=targetAgent.adapterOptions as {environmentKey?:string;managedGit?:boolean;continueFrom?:unknown}|undefined,sourceOptions=sourceAgent.adapterOptions as {environmentKey?:string;managedGit?:boolean}|undefined;if(targetConnection?.adapter!=="codex-runner"||sourceConnection?.adapter!=="codex-runner"||targetAgent.connectionKey!==sourceAgent.connectionKey||targetOptions?.environmentKey!==sourceOptions?.environmentKey||Boolean(targetOptions?.managedGit)!==Boolean(sourceOptions?.managedGit)||targetOptions?.continueFrom)throw new Error("conversation_source_incompatible")}
...
export async function validateWorkflowRunAdmissionSources(workflow:WorkflowDefinition,agents:readonly AgentDefinitionV1[],connectionIdentities:readonly {key:string;adapter:string}[],availableConnectionKeys:ReadonlySet<string>,managedEnvironmentKeys:ReadonlySet<string>){
 validatePublishGitHubPrSources(workflow,agents,connectionIdentities,managedEnvironmentKeys);
 await validateWorkflowReferences(workflow,agents,availableConnectionKeys);
}
