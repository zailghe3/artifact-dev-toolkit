...
function validateGraphTopology(workflow:{nodes:Array<{id:string;blockType:string;blockVersion:number;config:unknown}>;edges:Array<{source:string;sourcePort?:string;target:string;targetPort?:string}>},context:z.RefinementCtx){
 ...
 const entries=[...incoming].filter(([,edges])=>edges.length===0).map(([key])=>key),terminals=[...outgoing].filter(([,edges])=>edges.length===0).map(([key])=>key);if(entries.length!==1)context.addIssue({code:"custom",message:"Workflow must have exactly one graph entry.",path:["edges"]});if(terminals.length!==1)context.addIssue({code:"custom",message:"Workflow must have exactly one successful terminal.",path:["edges"]});
 for(const [nodeId,node] of byId){const ins=incoming.get(nodeId)!,outs=outgoing.get(nodeId)!;if(ins.length>1){const exclusive=[...byId.values()].some(candidate=>candidate.blockType==="condition"&&["true","false"].every(port=>{const start=outgoing.get(candidate.id)?.find(edge=>edge.sourcePort===port)?.target;if(!start)return false;const seen=new Set<string>();const visit=(id:string):boolean=>{if(ins.some(edge=>edge.source===id))return true;if(seen.has(id)||id===nodeId)return false;seen.add(id);return (outgoing.get(id)??[]).some(edge=>visit(edge.target))};return visit(start)}));if(!exclusive)context.addIssue({code:"custom",message:"Joins are not supported yet.",path:["edges"]});}if(node.blockType==="agent"&&outs.length>1)context.addIssue({code:"custom",message:"Agent fan-out is not supported yet.",path:["edges"]});if(node.blockType==="condition"){if(ins.length!==1)context.addIssue({code:"custom",message:"Condition must have exactly one input.",path:["nodes",workflow.nodes.indexOf(node)]});const ports=outs.map(edge=>edge.sourcePort);if(outs.length!==2||ports.filter(port=>port==="true").length!==1||ports.filter(port=>port==="false").length!==1)context.addIssue({code:"custom",message:"Condition must have exactly one true route and one false route.",path:["nodes",workflow.nodes.indexOf(node)]});}}
...
const genericPlanNode=z.object({id,blockType:z.enum(["agent","condition"]),blockVersion:z.literal(1),config:z.union([z.object({agentId:id}).strict(),z.object({operator:z.literal("contains"),value:z.string().min(1).max(4096),caseSensitive:z.boolean()}).strict()])}).strict();
