...
function scopedId(path:string[],semanticId:string){const raw=[...path,semanticId].join("-");if(raw.length<=80)return raw;let hash=2166136261;for(const char of raw)hash=Math.imul(hash^char.charCodeAt(0),16777619);return `${raw.slice(0,70)}-${(hash>>>0).toString(36)}`.slice(0,80)}
/** Deterministically replaces compact subworkflow nodes with frozen primitive child graphs. */
export function composeWorkflowV2(root:WorkflowDefinitionV2,dependencies:ReadonlyMap<string,WorkflowDefinitionV2>):WorkflowDefinitionV2{
 const expand=(workflow:WorkflowDefinitionV2,path:string[],stack:string[]):WorkflowDefinitionV2=>{if(stack.length>MAX_COMPOSITE_DEPTH)throw new Error("subworkflow_depth_exceeded");const replacements=new Map<string,{entry:string;terminal:string}>(),nodes:WorkflowDefinitionV2["nodes"]=[],edges:WorkflowDefinitionV2["edges"]=[];
  for(const node of workflow.nodes){const refs=workflowBlockRegistry.references(node.blockType,node.blockVersion,node.config).workflowIds??[];if(!refs.length){nodes.push({...node,id:scopedId(path,node.id)});continue}const childId=refs[0];if(stack.includes(childId))throw new Error(`subworkflow_cycle:${[...stack,childId].join("->")}`);const child=dependencies.get(childId);if(!child)throw new Error(`missing_subworkflow:${childId}`);if(!child.exposableAsBlock)throw new Error(`subworkflow_not_exposed:${childId}`);const expanded=expand(child,[...path,node.id],[...stack,childId]);const entry=graphEntryNode(expanded.nodes,expanded.edges),outgoing=new Set(expanded.edges.map(edge=>edge.source)),terminal=expanded.nodes.find(value=>!outgoing.has(value.id))!.id;replacements.set(node.id,{entry,terminal});nodes.push(...expanded.nodes);edges.push(...expanded.edges)}
  for(const edge of workflow.edges){const source=replacements.get(edge.source)?.terminal??scopedId(path,edge.source),target=replacements.get(edge.target)?.entry??scopedId(path,edge.target);edges.push({...edge,id:scopedId(path,edge.id),source,target})}return{...workflow,id:root.id,exposableAsBlock:root.exposableAsBlock,nodes,edges,limits:root.limits}};
 const composed=expand(root,[],[root.id]);if(composed.nodes.length>MAX_WORKFLOW_STEPS||composed.edges.length>MAX_STEP_EXECUTIONS)throw new Error("subworkflow_expansion_too_large");return workflowDefinitionV2Schema.parse(composed);
}
export function validateAndComposeWorkflow(workflow:WorkflowDefinition,definitions:readonly WorkflowDefinition[]){if(workflow.schemaVersion===1)return workflow;const dependencies=new Map(definitions.filter((value):value is WorkflowDefinitionV2=>value.schemaVersion===2).map(value=>[value.id,value]));return composeWorkflowV2(workflow,dependencies)}
