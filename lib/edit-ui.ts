...
export function hasPreview(value: unknown): value is { metadata: { id: string; title: string; type: string; status: string; sourceId?: string; createdAt?: string; tags: string[]; aliases: string[] }; bodyHtml: string } {
  const candidate = value as { metadata?: Record<string, unknown>; bodyHtml?: unknown } | null;
  return !!candidate && typeof candidate.bodyHtml === "string" && typeof candidate.metadata?.id === "string" && typeof candidate.metadata.title === "string" && typeof candidate.metadata.type === "string" && typeof candidate.metadata.status === "string" && (candidate.metadata.sourceId === undefined || typeof candidate.metadata.sourceId === "string") && (candidate.metadata.createdAt === undefined || typeof candidate.metadata.createdAt === "string") && Array.isArray(candidate.metadata.tags) && candidate.metadata.tags.every((value) => typeof value === "string") && Array.isArray(candidate.metadata.aliases) && candidate.metadata.aliases.every((value) => typeof value === "string");
}
