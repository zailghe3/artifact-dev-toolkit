...
export const OPENAI_AGENTS_MODEL_TIMEOUT_MS = 30_000;

export type OpenAIAgentsClientConfiguration = { apiKey: string; maxRetries: 0 };
export type OpenAIAgentsProviderConfiguration = { openAIClient: OpenAI; useResponses: true };
...
export type OpenAIAgentsRuntimeFactories = {
  createClient(configuration: OpenAIAgentsClientConfiguration): OpenAI;
...
const defaults: OpenAIAgentsRuntimeFactories = {
  createClient: configuration => new OpenAI(configuration),
...
class RuntimeFailure extends Error {
  ...
  constructor(category: FailureCategory, safeMessage: string) { super(category); this.category = category; this.safeMessage = safeMessage; }
}
function failure(category: FailureCategory, safeMessage: string): RuntimeFailure {
  return new RuntimeFailure(category, safeMessage);
}
...
function classifySDKFailure(error: unknown): RuntimeFailure {
  ...
  if (error instanceof ModelRefusalError) return failure("provider_rejected", "The model refused the request.");
...
export function openAIAgentsModelSettings(options: unknown): ModelSettings {
  ...
  return {
    ...
    timeoutMs: OPENAI_AGENTS_MODEL_TIMEOUT_MS,
...
export class OpenAIAgentsRuntime implements AgentRuntime {
  ...
  constructor(factories: Partial<OpenAIAgentsRuntimeFactories> = {}) { this.factories = { ...defaults, ...factories }; }
  ...
  async start(invocation: AdapterInvocation): Promise<AdapterResult> {
    ...
    const client = this.factories.createClient({ apiKey: connection.credential, maxRetries: 0 });
    const provider = this.factories.createProvider({ openAIClient: client, useResponses: true });
