...
export const connectionDefinitionSchema=z.object({
 schemaVersion:z.literal(1),id,name:z.string().trim().min(1).max(120),runtime:z.enum(["openai-responses","openai-agents"]),provider:z.literal("openai"),model:z.string().trim().min(1).max(120),credential:z.object({secretRef:z.string().regex(SECRET_BINDING)}).strict(),
