...
import {PROVIDER_CONNECTION_TYPE_IDS,getProviderConnectionType,normalizeProviderSafeConfiguration} from "./provider-connection-types.ts";
...
export const connectionDefinitionSchema=z.object({
 schemaVersion:z.literal(1),id,name:z.string().trim().min(1).max(CONNECTION_NAME_MAX_LENGTH),runtime:z.enum(PROVIDER_CONNECTION_TYPE_IDS),provider:z.string().trim().min(1).max(80),model:z.string().trim().min(1).max(120).optional(),configuration:z.unknown().optional(),credential:vaultCredential,
}).strict().superRefine((value,context)=>{if(RESERVED_CONNECTION_KEYS.has(value.id))context.addIssue({code:"custom",message:"Reserved connection IDs are not permitted.",path:["id"]});const type=getProviderConnectionType(value.runtime);if(!type||type.provider!==value.provider)context.addIssue({code:"custom",message:"Provider does not match connection type.",path:["provider"]});if(type?.model.required&&!value.model)context.addIssue({code:"custom",message:"Model is required for this connection type.",path:["model"]});if(type)try{normalizeProviderSafeConfiguration(type,value.configuration)}catch{context.addIssue({code:"custom",message:"Provider configuration is invalid.",path:["configuration"]})}}).transform(value=>{const type=getProviderConnectionType(value.runtime)!,configuration=normalizeProviderSafeConfiguration(type,value.configuration);const{schemaVersion,id,name,runtime,provider,model,credential}=value;return{schemaVersion,id,name,runtime,provider,...(model?{model}:{}),...(configuration?{configuration}:{}),credential}});
