...
type CredentialResolver=Pick<WorkflowProviderConnectionStore,"resolveForSnapshot">;
...
export async function resolveWorkflowRunConnectionSnapshot(connection:ConnectionDescriptor,providerStore:CredentialResolver):Promise<ConnectionDescriptor>{
 ...
 const resolved=await providerStore.resolveForSnapshot(connection.key,connection);
