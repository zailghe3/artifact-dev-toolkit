...
type CredentialResolver=Pick<WorkflowProviderConnectionStore,"resolveForExecution">;
...
export async function resolveWorkflowRunConnectionSnapshot(connection:ConnectionDescriptor,providerStore:CredentialResolver):Promise<ConnectionDescriptor>{
 ...
 const resolved=await providerStore.resolveForExecution(connection.key,connection);
