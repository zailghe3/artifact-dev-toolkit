...
export class GitAuthoritativeWorkflowProviderConnectionStore implements WorkflowProviderConnectionStore{
 ...
 async resolveCredential(key:string,snapshot?:ConnectionDescriptor):Promise<ResolvedConnection>{if(snapshot?.management==="git")return resolveGitSnapshotCredential(key,snapshot,this.secrets);const value=await this.git.getConnection(key);if(value)return{...this.descriptor(value,true),credential:this.secrets.resolve(value.definition.credential.secretRef)};return this.fallback.resolveCredential(key,snapshot)}
