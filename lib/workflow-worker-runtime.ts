...
export async function executeWorkflowGraphNode(env:CloudflareEnv,scope:GraphNodeAuthorityScope,inputText:string){if(!env.AUTH_SESSIONS_DB)throw new Error("workflow_storage_unavailable");const storage=new D1WorkflowRunStorage(env.AUTH_SESSIONS_DB as unknown as WorkflowD1Database),detail=await storage.getRun(scope.runId),attempt=detail?.attempts.filter(value=>value.stepId===scope.nodeId).at(-1);if(!detail||detail.run.engineVersion!=="2"||detail.run.workflowGeneration!==scope.workflowGeneration||!attempt||attempt.iteration!==scope.iteration||attempt.attempt!==scope.attempt)throw new Error("invalid_graph_node");const composition=executionComposition(env);return executeDurableGraphNodeTurn({runId:scope.runId,nodeId:scope.nodeId,inputText,storage,runtimes:composition.runtimes,resolveConnection:composition.resolveConnectionForRun})}
