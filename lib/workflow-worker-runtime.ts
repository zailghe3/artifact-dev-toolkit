...
import {durableRuntimeFailure,parseDurableLangGraphStepResult,type DurableRuntimeFailure} from "./workflow-durable-runtime-result.ts";
...
export async function executeLangGraphWorkflow(input:{runId:string;storage:import("./workflow-storage.ts").WorkflowRunStorage;remote?:LangGraphRemote;remoteFactory?:()=>LangGraphRemote;step:DurableStep;checkpointUrl:string;nodeUrl:string;checkpointAuthority:()=>Promise<string>;nodeAuthority:(scope:GraphNodeAuthorityScope)=>Promise<string>;suspendedMs?:()=>Promise<number>;approval?:{wait(result:Extract<LangGraphAdvanceResult,{state:"approval_required"}>|LangGraphAdvanceResult,generation:number):Promise<{state:"approved";requestId:string;interruptId:string}|{state:"cancelled"}>};now?:()=>number}){
 ...
 for(let turn=0;turn<MAX_LANGGRAPH_TRANSPORT_TURNS;turn++){
  ...
  if(current.run.engineVersion!=="2"||!current.run.executionPlan||!("planVersion" in current.run.executionPlan))throw new Error("legacy_run_read_only");const plan=current.run.executionPlan;
  ...
  try{
   const durableValue=await step.do(`${runId}:langgraph:${current.run.workflowGeneration}:${turn}`,NO_PLATFORM_RETRY,async()=>{
    try{
     const remote=input.remoteFactory?.()??input.remote!;
     const checkpointGateway={url:input.checkpointUrl,authority:await input.checkpointAuthority()},base={runId,initialInput:current.run.initialInput,plan,checkpointGateway,...(resume?{approvalResume:resume}:{})},inspection=await remote.advanceLangGraph({...base,nodeGateway:{url:input.nodeUrl}});
     if(inspection.state!=="admission_required")return{kind:"result" as const,result:inspection};
     const admitted=await storage.getRun(runId);if(!admitted||admitted.run.engineVersion!=="2"||!admitted.run.executionPlan||!("planVersion" in admitted.run.executionPlan)||admitted.run.workflowGeneration!==current.run.workflowGeneration)throw new Error("invalid_graph_admission");if(!inspection.frontier?.length)throw new Error("invalid_graph_admission");
     const authorities=await Promise.all(inspection.frontier.map(async activation=>{const existing=admitted.attempts.filter(value=>value.stepId===activation.nodeId&&value.graphActivationId===activation.activationId).at(-1),attempt=admitted.run.failureCode?(existing??(()=>{throw new Error("invalid_graph_admission")})()):await storage.ensureGraphActivation(runId,activation.nodeId,activation.activationId);if(!["pending","starting","waiting_provider","succeeded","failed"].includes(attempt.status))throw new Error("invalid_graph_admission");const scope={runId,nodeId:activation.nodeId,activationId:activation.activationId,workflowGeneration:admitted.run.workflowGeneration,iteration:attempt.iteration,attempt:attempt.attempt},authority=await input.nodeAuthority(scope);return{nodeId:scope.nodeId,activationId:scope.activationId,workflowGeneration:scope.workflowGeneration,iteration:scope.iteration,attempt:scope.attempt,authority}}));
     return{kind:"result" as const,result:await remote.advanceLangGraph({...base,nodeGateway:{url:input.nodeUrl,authorities}})};
    }catch(error){if(error instanceof RemoteRuntimeFailure)return durableRuntimeFailure(error);throw error}
   });
   const durable=parseDurableLangGraphStepResult(durableValue);if(!durable)throw new Error("invalid_durable_langgraph_result");if(durable.kind==="runtime_failure")throw durable.failure;result=durable.result;
  }
  catch(error){const reconciled=await reconcileOrchestrationRace(storage,runId,current.run.workflowGeneration);if(reconciled.done)return{runId,status:reconciled.status};const runtime=parseDurableLangGraphStepResult({kind:"runtime_failure",failure:error})?.kind==="runtime_failure"?error as DurableRuntimeFailure:undefined;failures++;const diagnostic=orchestrationDiagnostic(runtime,failures,new Date(now()).toISOString());if(runtime?.runtimeCode==="capability_unavailable"){await storage.failRun(runId,"connection_unavailable","ADT Runtime LangGraph capability is unavailable.",diagnostic,current.run.workflowGeneration);return{runId,status:"failed" as const}}if(runtime&&["authentication_failed","configuration_invalid"].includes(runtime.category)){await storage.failRun(runId,runtime.category,runtime.safeMessage,diagnostic,current.run.workflowGeneration);return{runId,status:"failed" as const}}if(failures>MAX_LANGGRAPH_RECOVERY_FAILURES){await storage.failRun(runId,"provider_unavailable","Workflow orchestration remained unavailable after bounded recovery.",diagnostic,current.run.workflowGeneration);return{runId,status:"failed" as const}}const mutation=await storage.recordOrchestrationDiagnostics(runId,current.run.workflowGeneration,diagnostic);if(mutation!=="updated"){const changed=await reconcileOrchestrationRace(storage,runId,current.run.workflowGeneration);if(changed.done)return{runId,status:changed.status};throw new Error("diagnostic_mutation_conflict")}await step.sleep(`${runId}:langgraph:${current.run.workflowGeneration}:recovery:${turn}`,recoveryDelay(failures));continue}
  ...
  if(result.state==="graph_failure"){await storage.failRun(runId,result.failureCode??"internal_error",result.safeMessage??"Workflow graph execution failed.");return{runId,status:"failed" as const}}
