...
export async function executeLangGraphWorkflow(input:{runId:string;storage:import("./workflow-storage.ts").WorkflowRunStorage;remote:LangGraphRemote;step:DurableStep;checkpointUrl:string;nodeUrl:string;checkpointAuthority:()=>Promise<string>;nodeAuthority:(scope:GraphNodeAuthorityScope)=>Promise<string>;suspendedMs?:()=>Promise<number>;approval?:{wait(result:Extract<LangGraphAdvanceResult,{state:"approval_required"}>|LangGraphAdvanceResult,generation:number):Promise<{state:"approved";requestId:string;interruptId:string}|{state:"cancelled"}>};now?:()=>number}){
 const {runId,storage,remote,step}=input,now=input.now??Date.now;let failures=0,resume:LangGraphAdvanceRequest["approvalResume"];
