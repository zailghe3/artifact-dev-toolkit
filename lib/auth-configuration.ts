...
function requireValues<const T extends readonly string[]>(names: T): Record<T[number], string> {
...
export function getOAuthStartConfig() {
  const values = requireValues(["GITHUB_APP_CLIENT_ID"] as const);
  return { clientId: values.GITHUB_APP_CLIENT_ID };
}

export function getOAuthExchangeConfig() {
  const values = requireValues(["GITHUB_APP_CLIENT_ID", "GITHUB_APP_CLIENT_SECRET"] as const);
...
export function getSessionSecurityConfig() {
  const values = requireValues(["SESSION_SECRET", "GITHUB_TOKEN_ENCRYPTION_KEY"] as const);
...
export function validateProductionAuthReadiness() {
  requireValues(["GITHUB_APP_ID", "GITHUB_APP_CLIENT_ID", "GITHUB_APP_CLIENT_SECRET", "GITHUB_APP_PRIVATE_KEY", "GITHUB_TOKEN_ENCRYPTION_KEY", "SESSION_SECRET"] as const);
