...
export type AdapterInvocation = { runId: string; stepId: string; iteration: number; attempt: number; providerPollCount: number; idempotencyKey: string; agentName: string; masterPrompt: string; inputText: string; tools?: readonly "artifact_search"[]; repositoryContext?:RunRepositoryContext; managedCodeRepositoryContext?:ManagedCodeRepositoryContext; connection: ResolvedConnection; providerOptions?: unknown };
...
export function validateAdapterOptions(adapter:string,options:unknown){if(adapter==="deterministic-test")return deterministicOptionsSchema.parse(options??{});if(adapter==="openai-responses")return openAIResponsesOptionsSchema.parse(options??{});if(adapter==="openai-agents")return openAIAgentsOptionsSchema.parse(options??{});if(adapter==="codex-runner")return codexRunnerOptionsSchema.parse(options);if(adapter==="codex-cloud")return codexCloudOptionsSchema.parse(options);throw new Error("unsupported_adapter");}
