...
export function runnerActionFailure(stage:RunnerActionStage,error:unknown,logger:(message:string)=>void=console.error){
 ...
 logger(JSON.stringify({event:"codex_runner_action_failed",stage,category,...(error instanceof CodexRunnerError&&error.transport?{transport:error.transport}:{}),...(runnerCode?{runnerCode}:{})}));
 return{error:category,...(runnerCode?{runnerCode}:{})};
}
