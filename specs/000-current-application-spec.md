...
The protected lifecycle editor creates only draft artifacts, normalizes tag and alias lists, and keeps existing IDs immutable. Draft and archived edits are direct exact-SHA commits. The repository rejects direct production updates even if a caller bypasses the UI; those changes use the proposal workflow.

Every deletion control opens an accessible confirmation before a request is made. Draft and archived deletion resolves the exact current tree path and blob SHA, sends one Contents API DELETE, and invalidates the catalogue after success. Production deletion is rejected by the direct endpoint and instead creates a deterministic `artifact-delete/{id}-{sha-prefix}` Git Data tree removal and pull request without invalidating the unchanged base catalogue. Stale revisions fail before mutation, and incomplete proposals expose only a validated recovery branch URL.
