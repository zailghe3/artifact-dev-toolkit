...
Codex remains selectable while creating or editing an Agent even when it is not presently executable, so the editor can show specific readiness failures. Runner reachability, capability-catalogue availability, Workflow job capability, environment-catalogue availability, individual environment readiness, ChatGPT authentication, and model-catalogue availability are independent dimensions. The Runner's `/v1/environments` response is the exclusive authoritative catalogue for `codex-runner`; legacy D1 Codex Cloud environment references are unrelated. The editor retains unavailable saved values for diagnosis, but both save and execution validation fail closed against the live Runner catalogue.

Normal ADT Workflows can invoke that adapter against an operator-preconfigured local Runner environment. An Agent persists only `environmentKey` plus optional live-discovered `model` and `reasoningEffort`; the Runner privately maps the environment key to a canonical directory and a `read-only` or `workspace-write` sandbox. It starts one Codex App Server thread and one turn with `approvalPolicy: "never"`, then exposes only the final agent-message text (at most 262,144 UTF-8 bytes) as the Workflow step output. Commands and file changes are normal job activity within the configured sandbox, while interactive or approval requests fail closed.

Agent definitions store the Runner's public environment key, never a browser-supplied `cwd`, absolute path, repository mount, sandbox setting, Runner credential, secret, or ChatGPT authentication material. Deployment, restart, image rollout, Portainer configuration, mounts, and lifecycle of the running home-lab Runner are operator-owned and outside ADT deployment requirements. ADT CI may build and publish an image, but never deploys or restarts that service.

Runner jobs use a durable idempotency digest and request fingerprint, an opaque job ID, atomic restrictive state files, and a single global execution lease. A matching replay returns the same job; a conflicting replay cannot execute. Active persisted jobs become `runner_restarted` after process restart and are never automatically resubmitted. Cancellation remains a non-terminal Workflow `cancelling` state while the Runner reports `cancellation_pending`; bounded durable sleeps reissue cancellation against the same task until a definitive result, and transient polling outages retain that accepted provider task. The v1 slice does not clone or provision repositories, commit, push, publish pull requests, or configure MCP. The old Codex Cloud/D1 environment concept remains separate, deprecated, and unavailable.

Only `POST /v1/jobs` receives the dedicated 1,970,000-byte HTTP envelope. That bound covers worst-case JSON escaping of the existing 65,536-character master prompt, 262,144-byte Workflow input, fixed framing, and bounded request identity/model fields; the parsed composed prompt remains capped at 524,320 UTF-8 bytes. Authentication, diagnostics, testing, and other control routes retain their smaller limits.
