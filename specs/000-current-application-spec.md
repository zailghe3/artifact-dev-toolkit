...
Runner jobs use a durable idempotency digest and request fingerprint, an opaque job ID, atomic restrictive state files, and a single global execution lease. A matching replay returns the same job; a conflicting replay cannot execute. Active persisted jobs become `runner_restarted` after process restart and are never automatically resubmitted. Cancellation may be reported as pending until the exact turn has quiesced, and transient polling outages retain the same accepted provider task. The v1 slice does not clone or provision repositories, commit, push, publish pull requests, or configure MCP. The old Codex Cloud/D1 environment concept remains separate, deprecated, and unavailable.
