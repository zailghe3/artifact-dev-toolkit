...
Device authentication requires the Codex subprocess—not merely the inbound Cloudflare bridge—to have outbound DNS and TCP/TLS access to `auth.openai.com:443`. An `internal` Docker network needs a separate egress path. TLS-intercepting networks may select a PEM bundle through `CODEX_CA_CERTIFICATE` or the lower-precedence `SSL_CERT_FILE`, while proxy-controlled environments may require HTTP/HTTPS proxy configuration; these are optional deployment concerns and their paths, values, and credentials are never logged. An authenticated ADT action proxies the Runner's fixed-host, non-mutating auth-environment diagnostic and displays only bounded readiness, DNS, TCP, proxy-presence, and custom-CA states. It exposes no addresses, paths, environment values, raw errors, or arbitrary probing.
