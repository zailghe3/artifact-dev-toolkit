...
- Readiness separately advertises `openai-agents`, `execution-timeout`, `tool:artifact-search`, `langgraph:graph`, and the optional `diagnostic:execution-path` capability; ADT rejects capability-specific execution before provider use when the required capability is absent.
...
- The optional execution timeout is strictly bounded to 5–120 seconds and defaults to 30 seconds for older callers. The Runtime keeps inbound request and header limits narrow, while a fixed 150-second socket-response ceiling permits the maximum 126-second control-plane transport allowance without exposing another user setting. A control plane requires the `execution-timeout` capability before sending a non-default setting.
