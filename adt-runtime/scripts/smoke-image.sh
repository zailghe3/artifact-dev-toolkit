...
const response=await fetch(`http://127.0.0.1:8080${path}`,{headers:{'x-adt-protocol':protocol,'x-adt-timestamp':timestamp,'x-adt-nonce':nonce,'x-adt-content-sha256':digest,'x-adt-signature':signature}}),value=await response.json();if(!response.ok||value.runtimeRevision!==process.env.EXPECTED_REVISION||value.protocolVersion!==protocol||!value.capabilities.includes('openai-agents')||!value.capabilities.includes('tool:artifact-search')||!value.capabilities.includes('langgraph:linear')||!value.credentialWrappingKeyId)process.exit(1);
NODE
