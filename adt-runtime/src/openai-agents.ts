import {Agent,MaxTurnsExceededError,ModelBehaviorError,ModelRefusalError,ModelTimeoutError,OpenAIProvider,Runner,UserError,tool,type ModelProvider,type ModelSettings,type Tool} from "@openai/agents";
import {z} from "zod";
import OpenAI from "openai";
import type {ExecutionRequest} from "./protocol.js";

export const MAX_TURNS=6, MAX_TOOL_CALLS=4, DEFAULT_MODEL_TIMEOUT_MS=30_000,MODEL_TIMEOUT_MS=DEFAULT_MODEL_TIMEOUT_MS, MAX_OUTPUT_BYTES=262_144,MAX_TOOL_RESULT_BYTES=128_000;
...
export type Factories={client:(c:{apiKey:string;maxRetries:0})=>OpenAI;provider:(c:{openAIClient:OpenAI;useResponses:true})=>ModelProvider&{close?:()=>Promise<void>};runner:(c:{modelProvider:ModelProvider;tracingDisabled:true})=>Pick<Runner,"run">;agent:(c:{name:string;instructions:string;model:string;modelSettings:ModelSettings;tools?:Tool<unknown>[]})=>Agent<unknown,"text">;fetcher:typeof fetch};
...
export async function executeOpenAIAgents(request:ExecutionRequest,credential:string,overrides:Partial<Factories>={}){
 ...
 const o=request.options,modelSettings:ModelSettings={store:false,parallelToolCalls:false,timeoutMs:request.executionTimeoutMs??DEFAULT_MODEL_TIMEOUT_MS,...(o.maxOutputTokens?{maxTokens:o.maxOutputTokens}:{}),...(o.reasoningEffort?{reasoning:{effort:o.reasoningEffort}}:{}),...(o.verbosity?{text:{verbosity:o.verbosity}}:{})};
 let toolCalls=0;const tools=(request.tools??[]).length?[tool({name:"artifact_search",description:"Search the authorised ADT Artifact Library.",parameters:z.object({query:z.string().min(1).max(500),limit:z.number().int().min(1).max(10).optional()}).strict(),execute:async (args,_context,details)=>{if(++toolCalls>MAX_TOOL_CALLS)throw fail("provider_rejected","The Agent exceeded the permitted tool-call limit.");const response=await f.fetcher(request.toolGateway!.url,{method:"POST",headers:{authorization:`Bearer ${request.toolGateway!.authority}`,"content-type":"application/json"},body:JSON.stringify({runId:request.idempotencyKey.split(":",1)[0],tool:"artifact_search",callId:details?.toolCall?.callId??`call-${toolCalls}`,arguments:args})});const value=await response.text();if(!response.ok||Buffer.byteLength(value,"utf8")>MAX_TOOL_RESULT_BYTES)throw fail("provider_rejected","Artifact search failed safely.");return value;}})]:[];
