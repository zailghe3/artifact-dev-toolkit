import {Agent,MaxTurnsExceededError,ModelBehaviorError,ModelRefusalError,ModelTimeoutError,OpenAIProvider,Runner,UserError,type ModelProvider,type ModelSettings} from "@openai/agents";
...
export type Factories={client:(c:{apiKey:string;maxRetries:0})=>OpenAI;provider:(c:{openAIClient:OpenAI;useResponses:true})=>ModelProvider&{close?:()=>Promise<void>};runner:(c:{modelProvider:ModelProvider;tracingDisabled:true})=>Pick<Runner,"run">;agent:(c:{name:string;instructions:string;model:string;modelSettings:ModelSettings})=>Agent<unknown,"text">};
const defaults:Factories={client:c=>new OpenAI(c),provider:c=>new OpenAIProvider(c),runner:c=>new Runner(c),agent:c=>new Agent(c)};
