...
test('Codex Runner snapshot persists only safe metadata without provider resolution',async()=>{
 ...
 const snapshot=await resolveWorkflowRunConnectionSnapshot(connection,{resolveForExecution:async()=>{calls++;throw new Error('must not resolve')}});
...
test('OpenAI Responses snapshot verifies the stored credential but never persists it',async()=>{
 ...
 const snapshot=await resolveWorkflowRunConnectionSnapshot(connection,{resolveForExecution:async key=>{calls++;assert.equal(key,'openai-primary');return{...connection,credential,privateOptions:{hidden:true}}}});
...
test('mixed GPT then Codex run snapshots resolve successfully across both trust boundaries',async()=>{
 let calls=0;const openai=descriptor('openai-primary','openai-responses',{defaultModel:'gpt-safe'}),codex=descriptor('codex-primary','codex-runner',{serverConfiguration:{sharedSecret:'runner-secret'}}),store={resolveForExecution:async key=>{calls++;assert.equal(key,openai.key);return{...openai,credential:'openai-secret'}}};
