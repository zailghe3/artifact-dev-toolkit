...
test('Workspace status renders advisory Git states and never calls unknown clean',()=>{const React=require('react'),{renderToStaticMarkup}=require('react-dom/server'),{WorkspaceSection}=require('../components/CodexRunnerOperationalStatus.tsx'),environment={key:'artifact-dev-toolkit',name:'Artifact Dev Toolkit',enabled:true,ready:true,sandbox:'workspace-write'};const unavailable=renderToStaticMarkup(React.createElement(WorkspaceSection,{items:[{environment,diagnostics:null}]}));assert.match(unavailable,/require a newer Runner/);const nonGit=renderToStaticMarkup(React.createElement(WorkspaceSection,{items:[{environment,diagnostics:{environmentKey:environment.key,filesystemReady:true,gitAvailable:true,gitRepository:false,headCommit:null,dirty:null}}]}));assert.match(nonGit,/Not a Git checkout/);assert.match(nonGit,/repository-aware operations are unavailable/);assert.match(nonGit,/State unavailable/);assert.doesNotMatch(nonGit,/>Clean</)});
