...
test("auth diagnostic proxy uses the fixed Runner route and validates only bounded fields",async()=>{
 ...
 const legacy={...diagnostic};delete legacy.codexNativeLibc;delete legacy.codexAddressPolicyApplies;delete legacy.codexAuthTransportProbeStatus;delete legacy.codexAuthTransportProbe;assert.deepEqual(await new CodexRunnerClient(configuration,async()=>Response.json(legacy)).authEnvironmentDiagnostics(),legacy);
