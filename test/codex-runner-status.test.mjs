...
const capabilities={protocolVersion:1,runnerRevision:EXPECTED_RUNNER_RELEASE.runnerRevision,runnerVersion:"a".repeat(40),codexVersion:"0.153.4",codexAvailable:true,deviceAuth:true,jobExecution:true};
...
test("capabilities parser rejects malformed release metadata and unsupported protocol",async()=>{
 ...
 for(const value of [{...capabilities,runnerRevision:undefined},{...capabilities,runnerRevision:0},{...capabilities,runnerVersion:"abc123"},{...capabilities,codexVersion:"0.153"},{...capabilities,privateConfiguration:"secret"}])await assert.rejects(client(value).capabilities(),error=>error.category==="invalid_response");
...
test("capabilities accepts a caller deadline without changing its normal timeout",async()=>{
 const caller=new AbortController();let requestSignal;
 const client=new CodexRunnerClient(configuration,async(_url,options)=>{requestSignal=options.signal;return new Promise((_resolve,reject)=>options.signal.addEventListener("abort",()=>reject(new Error("aborted")),{once:true}))});
 const result=client.capabilities(caller.signal);await Promise.resolve();
 assert.equal(requestSignal.aborted,false);caller.abort();assert.equal(requestSignal.aborted,true);
...
test("reduced diagnostics reject every non-finite or malformed field",async()=>{const valid={runnerReachable:true,codexAppServerReady:true,codexVersion:"0.153.4",codexVersionMatchesExpected:true,codexNativeLibc:"glibc",codexAddressPolicyApplies:true,customCaSource:"none",systemCaBundlePresent:true,systemCaBundleReadable:true,systemCaBundleNonEmpty:true,httpProxyConfigured:false,httpsProxyConfigured:false,allProxyConfigured:false,noProxyConfigured:false,dnsResolution:"ok",ipv4Available:true,ipv6Available:false,systemResolverFirstFamily:"ipv4",runnerAddressPolicy:"ipv4_preferred",runnerAddressPolicyEffective:false,kernelIpv6Disabled:false,ipv4TcpConnectivity:"ok",ipv6TcpConnectivity:"unavailable",ipv4TlsConnectivity:"ok",ipv6TlsConnectivity:"unavailable",deviceAuthRoute:{responseReceived:true,status:405},codexHomeReadable:true,codexHomeWritable:true,summary:[]};for(const patch of [{ipv4TlsConnectivity:"raw"},{ipv4TcpFailureReason:"raw"},{systemResolverFirstFamily:"raw"},{runnerAddressPolicy:"raw"},{codexNativeLibc:"raw"},{customCaSource:"raw"},{customCaFileReadable:true},{codexVersion:"0.153"},{summary:["ok",42]},{deviceAuthRoute:{responseReceived:true,status:99}},{deviceAuthRoute:{responseReceived:true,status:405,body:"secret"}},{systemCaBundlePresent:"yes"}])await assert.rejects(new CodexRunnerClient(configuration,async()=>Response.json({...valid,...patch})).authEnvironmentDiagnostics(),error=>error.category==="invalid_response")});
...
test("SQLite storage client strictly validates nested status and backup metadata",async()=>{const backup={backupId:"b-20260909T010203Z-abcdef123456",createdAt:"2026-09-09T01:02:03.000Z",runnerVersion:"a".repeat(40),codexVersion:"0.153.4",databases:[{filename:"state.sqlite",size:42,sha256:"b".repeat(64)}]},status={separateSqliteHomeConfigured:true,sqliteFilesystemClass:"local",sqliteHomeReadable:true,sqliteHomeWritable:true,codexHomeFilesystemClass:"network",backupDirectoryAvailable:true,backupDirectoryWritable:true,lastSuccessfulBackupAt:backup.createdAt,latestBackupId:backup.backupId,retainedBackupCount:1,retention:14,operation:"idle"};assert.deepEqual(await new CodexRunnerClient(configuration,async()=>Response.json({status,backups:[backup]})).sqliteStorage(),{status,backups:[backup]});for(const body of [{status:{...status,privatePath:"/secret"},backups:[backup]},{status:{...status,sqliteFilesystemClass:"nfs"},backups:[backup]},{status,backups:[{...backup,databases:[{...backup.databases[0],filename:"../auth.json"}]}]},{status,backups:[{...backup,databases:[{...backup.databases[0],contents:"secret"}]}]}])await assert.rejects(new CodexRunnerClient(configuration,async()=>Response.json(body)).sqliteStorage(),error=>error.category==="invalid_response")});

test("explicit diagnostics strictly validate startup and storage nested objects",async()=>{const base={runnerReachable:true,codexAppServerReady:false,codexVersion:"0.153.4",codexVersionMatchesExpected:true,codexNativeLibc:"glibc",codexAddressPolicyApplies:true,customCaSource:"none",systemCaBundlePresent:true,systemCaBundleReadable:true,systemCaBundleNonEmpty:true,httpProxyConfigured:false,httpsProxyConfigured:false,allProxyConfigured:false,noProxyConfigured:false,dnsResolution:"ok",ipv4Available:true,ipv6Available:false,systemResolverFirstFamily:"ipv4",runnerAddressPolicy:"ipv4_preferred",runnerAddressPolicyEffective:false,kernelIpv6Disabled:false,ipv4TcpConnectivity:"ok",ipv6TcpConnectivity:"unavailable",ipv4TlsConnectivity:"ok",ipv6TlsConnectivity:"unavailable",deviceAuthRoute:{responseReceived:true,status:405},codexHomeReadable:true,codexHomeWritable:true,storage:{codexHomeFilesystemClass:"network",sqliteHomeConfigured:true,sqliteHomeFilesystemClass:"local",sqliteHomeReadable:true,sqliteHomeWritable:true,tmpWritable:true,runMounted:true,backupDirectoryReadable:true,backupDirectoryWritable:true},appServerStartup:{category:"initialize_timeout",elapsedMs:8000,stderrTail:["safe"]},summary:[]};assert.deepEqual(await new CodexRunnerClient(configuration,async()=>Response.json(base)).authEnvironmentDiagnostics(),base);for(const patch of [{appServerStartup:{...base.appServerStartup,stderrTail:Array(13).fill("safe")}},{appServerStartup:{...base.appServerStartup,raw:"secret"}},{storage:{...base.storage,mountPath:"/private"}},{storage:{...base.storage,sqliteHomeFilesystemClass:"nfs"}}])await assert.rejects(new CodexRunnerClient(configuration,async()=>Response.json({...base,...patch})).authEnvironmentDiagnostics(),error=>error.category==="invalid_response")});

test("allowlisted storage operation failures remain bounded",async()=>{for(const [code,status] of [["runner_busy",409],["backup_not_found",404],["invalid_backup",409],["sqlite_backup_failed",503],["sqlite_backup_insufficient_space",503],["app_server_shutdown_unconfirmed",503],["restore_failed_rolled_back",503],["restore_rollback_failed",503],["storage_operation_result_unknown",503],["sqlite_home_not_configured",409]])await assert.rejects(new CodexRunnerClient(configuration,async()=>Response.json({error:code},{status})).restoreSqliteBackup("b-20260909T010203Z-abcdef123456"),error=>error.runnerCode===code);await assert.rejects(new CodexRunnerClient(configuration,async()=>Response.json({error:"raw_private_error",private:"secret"},{status:503})).restoreSqliteBackup("b-20260909T010203Z-abcdef123456"),error=>error.runnerCode===undefined&&error.category==="runner_unavailable")});
test("operational job list accepts conversation_unavailable without relaxing unknown failures",async()=>{const id='c'.repeat(48),digest='d'.repeat(64),now='2026-09-19T00:00:00.000Z',body={capacity:{maxActive:1,activeJobId:null},jobs:[{jobId:id,idempotencyDigest:digest,environmentKey:'dev',state:'failed',reason:'conversation_unavailable',createdAt:now,updatedAt:now}]};assert.deepEqual((await new CodexRunnerClient(configuration,async()=>Response.json(body)).jobs()).jobs[0].reason,'conversation_unavailable');await assert.rejects(new CodexRunnerClient(configuration,async()=>Response.json({...body,jobs:[{...body.jobs[0],reason:'unknown_private_reason'}]})).jobs(),error=>error.category==='invalid_response')});
