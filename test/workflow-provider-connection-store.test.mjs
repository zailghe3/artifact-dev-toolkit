...
test('migration retirement conditionally deletes only the exact persisted D1 provider row',async t=>{const{mf,db,store}=await fixture();t.after(()=>mf.dispose());await db.prepare('ALTER TABLE workflow_provider_connections ADD COLUMN display_name TEXT').run();await store.upsertConnection({connectionKey:'openai-migrate',displayName:'Migration source',adapter:'openai-responses',model:'migration-model',credential});await store.upsertConnection({connectionKey:'openai-keep',displayName:'Keep',adapter:'openai-responses',model:'keep-model',credential:'keep-unit-sentinel'});const safe=await store.getPersistedSafeDescriptor('openai-migrate');assert.doesNotMatch(JSON.stringify(safe),/credential|ciphertext|iv-sentinel/i);await assert.rejects(store.retirePersistedConnection({...safe,defaultModel:'changed-model'}),/migration_configuration_mismatch/);assert.ok(await db.prepare('SELECT connection_key FROM workflow_provider_connections WHERE connection_key=?').bind('openai-migrate').first());await store.retirePersistedConnection(safe);assert.equal(await db.prepare('SELECT connection_key FROM workflow_provider_connections WHERE connection_key=?').bind('openai-migrate').first(),null);assert.ok(await db.prepare('SELECT connection_key FROM workflow_provider_connections WHERE connection_key=?').bind('openai-keep').first())});
