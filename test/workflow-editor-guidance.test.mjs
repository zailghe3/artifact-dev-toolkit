...
test('Anthropic Agent settings expose only bounded maximum output tokens',()=>{const anthropic={key:'anthropic',name:'Claude Production',adapter:'anthropic-messages',defaultModel:'claude-model',enabled:true,capabilities:{asynchronous:false,cancellation:false}},html=render(React.createElement(WorkflowAgentEditor,{connections:[anthropic],initial:{id:'claude',name:'Claude',description:'',prompt:{source:'custom',text:'Act.'},connectionKey:'anthropic',adapterOptions:{maxOutputTokens:1234}}}));assert.match(html,/Maximum output tokens/);assert.match(html,/<input(?=[^>]*name="maxOutputTokens")(?=[^>]*min="1")(?=[^>]*max="32768")(?=[^>]*value="1234")/);for(const absent of ['Reasoning effort','Verbosity','Artifact search','Execution timeout'])assert.doesNotMatch(html,new RegExp(absent,'i'))});
