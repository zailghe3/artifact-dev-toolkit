...
test('saved Codex values remain explicit when authentication catalog is unavailable',()=>{const codex={key:'codex-primary',name:'Codex',adapter:'codex-runner',enabled:true,capabilities:{asynchronous:true,cancellation:true}},html=render(React.createElement(WorkflowAgentEditor,{connections:[codex],initial:{id:'saved',name:'Saved',description:'',masterPrompt:'Act.',connectionKey:'codex-primary',adapterOptions:{environmentKey:'old-env',model:'old-model',reasoningEffort:'old-effort'}}}));for(const value of ['old-env (saved; live validation unavailable)','old-model (saved; live validation unavailable)','old-effort (saved; unsupported)','Codex model catalog unavailable'])assert.ok(html.includes(value),value)});
