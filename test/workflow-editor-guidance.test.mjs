...
test('saved missing Codex model remains visible and blocks save',()=>{const html=render(React.createElement(WorkflowAgentEditor,{connections:[codexConnection],initial:{id:'saved',name:'Saved',description:'',masterPrompt:'Act.',connectionKey:'codex-primary',adapterOptions:{environmentKey:'dev',model:'old-model'}},codexSnapshot:snapshot()}));assert.match(html,/old-model \(saved; live validation unavailable\)/);assert.match(html,/The selected Codex model is unavailable\./);assert.match(html,/<button disabled=""[^>]*>Save Agent<\/button>/)});
test('saved unsupported Codex reasoning effort remains visible and blocks save',()=>{const html=render(React.createElement(WorkflowAgentEditor,{connections:[codexConnection],initial:{id:'saved',name:'Saved',description:'',masterPrompt:'Act.',connectionKey:'codex-primary',adapterOptions:{environmentKey:'dev',model:'model-a',reasoningEffort:'xhigh'}},codexSnapshot:snapshot()}));assert.match(html,/xhigh \(saved; unsupported\)/);assert.match(html,/The selected reasoning effort is unavailable for this Codex model\./);assert.match(html,/<button disabled=""[^>]*>Save Agent<\/button>/)});
test('Codex defaults have no model or reasoning readiness blocker',()=>{const html=render(React.createElement(WorkflowAgentEditor,{connections:[codexConnection],initial:{id:'saved',name:'Saved',description:'',masterPrompt:'Act.',connectionKey:'codex-primary',adapterOptions:{environmentKey:'dev'}},codexSnapshot:snapshot()}));assert.doesNotMatch(html,/selected Codex model is unavailable|selected reasoning effort is unavailable/);assert.doesNotMatch(html,/<button disabled=""[^>]*>Save Agent/)});
test('advertised Codex model and effort have no model or reasoning readiness blocker',()=>{const html=render(React.createElement(WorkflowAgentEditor,{connections:[codexConnection],initial:{id:'saved',name:'Saved',description:'',masterPrompt:'Act.',connectionKey:'codex-primary',adapterOptions:{environmentKey:'dev',model:'model-a',reasoningEffort:'high'}},codexSnapshot:snapshot()}));assert.doesNotMatch(html,/selected Codex model is unavailable|selected reasoning effort is unavailable/);assert.doesNotMatch(html,/<button disabled=""[^>]*>Save Agent/)});
