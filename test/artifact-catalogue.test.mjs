...
test('own uncertain pointer is cleaned up, never reused, and cleanup failures log safely', async () => {
  const cache = new MemoryCatalogueCache(); const repo = repository(); const wrapped = { get: cache.get.bind(cache), delete: cache.delete.bind(cache), put: async (key, value) => { await cache.put(key, value); if (key.endsWith(':current')) throw new Error('uncertain write'); } };
  const catalogue = service(repo, wrapped, { value: '2026-08-02T00:00:00.000Z' }); const result = await catalogue.list(); assert.equal(result.cacheState, 'degraded'); assert.equal([...cache.values.keys()].some(key => key.endsWith(':current')), false);
  await catalogue.list(); assert.equal(repo.calls.catalogues, 2);

  const failingCache = new MemoryCatalogueCache(); const logs = []; const secret = 'github_pat_cleanup_secret'; const body = 'private artifact body'; let pointerAttempted = false;
  const failing = { get: failingCache.get.bind(failingCache), delete: async () => { throw new Error(secret); }, put: async (key, value) => { if (key.endsWith(':generation') && pointerAttempted) throw new Error(body); await failingCache.put(key, value); if (key.endsWith(':current')) { pointerAttempted = true; throw new Error('uncertain write'); } } };
  const failedCleanup = await service(repository(), failing, { value: '2026-08-02T00:00:00.000Z' }, { info: value => logs.push(value), error: value => logs.push(value) }).list(); assert.equal(failedCleanup.cacheState, 'degraded'); assert.ok(logs.some(value => value.includes('verification_cleanup_failure'))); assert.ok(logs.every(value => !value.includes(secret) && !value.includes(body)));
});
