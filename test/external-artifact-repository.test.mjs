...
test('connection namespace validates canonical non-secret definitions and remains optional', async()=>{
 ...
 await writeFile(path.join(root,'connections','openai-primary.connection.json'),JSON.stringify({schemaVersion:1,id:'openai-primary',name:'OpenAI Responses',runtime:'openai-responses',provider:'openai',model:'gpt-5',credential:{secretRef:'WORKFLOW_PROVIDER_CONNECTION_OPENAI_PRIMARY'}}));
 assert.equal((await validateExternalArtifactRepository(root)).valid,true);
