...
test("production readiness validates every setting, secret shape, backend, and private-key import", async () => {
  ...
  for (const values of [{ SESSION_SECRET: "short-secret" }, { GITHUB_TOKEN_ENCRYPTION_KEY: "not base64!" }, { GITHUB_TOKEN_ENCRYPTION_KEY: Buffer.alloc(31).toString("base64") }, { GITHUB_APP_PRIVATE_KEY: "bad-pem" }]) {
  ...
  await withEnvironment({ ARTIFACT_REPOSITORY: "file" }, () => assert.rejects(config.validateProductionAuthReadiness(), error => error.code === "invalid_repository_backend" && error.missingNames === undefined));
});
...
test("OAuth start route redirects safely for valid, configuration, malformed, and unexpected outcomes", async () => {
  ...
  for (const name of failures) {
    ...
    assert.deepEqual(JSON.parse(logs.at(-1)), { event: "github_oauth_start_failed", category: "configuration", configurationCode: "missing_configuration", missingNames: [name] });
  }
  for (const code of ["invalid_session_secret", "invalid_encryption_key_format", "invalid_encryption_key_length", "invalid_repository_backend", "invalid_private_key"]) {
    ...
    assert.deepEqual(JSON.parse(logs.at(-1)), { event: "github_oauth_start_failed", category: "configuration", configurationCode: code });
  }
...
test("OAuth callback route returns secret-free redirects and diagnostic categories for every failure stage", async () => {
  const cases = [
    ["configuration", { stateError: new config.AuthenticationConfigurationError("missing_configuration", ["SESSION_SECRET"]) }, "configuration", "missing_configuration"],
    ...
    ["repository_configuration", { exchangeError: new config.AuthenticationConfigurationError("invalid_private_key") }, "configuration", "invalid_private_key"],
    ["session_configuration", { sessionError: new config.AuthenticationConfigurationError("invalid_session_secret") }, "configuration", "invalid_session_secret"],
  ...
  for (const [category, setup, expected, configurationCode] of cases) {
    const logs = [];
    const handler = routes.createOAuthCallbackRouteHandler({
      consumeOAuthState: async () => { if (setup.stateError) throw setup.stateError; return setup.state ?? { valid: true, returnTo: "/artifacts/one", pkceVerifier: "verifier-secret" }; },
    ...
    assert.equal(response.status, 307); assert.equal(location(response).searchParams.get("error"), expected);
    const diagnostic = JSON.parse(logs.at(-1)); assert.equal(diagnostic.category, category); assert.equal(diagnostic.configurationCode, configurationCode);
    if (configurationCode === "missing_configuration") assert.deepEqual(diagnostic.missingNames, ["SESSION_SECRET"]);
...
test("Wrangler structurally declares exactly the intended secrets and production repository", () => {
  ...
  assert.deepEqual(wrangler.secrets.required, ["GITHUB_APP_ID", "GITHUB_APP_CLIENT_ID", "GITHUB_APP_CLIENT_SECRET", "GITHUB_APP_PRIVATE_KEY", "GITHUB_TOKEN_ENCRYPTION_KEY", "WORKFLOW_PROVIDER_SECRET_ENCRYPTION_KEY", "SESSION_SECRET", "CODEX_RUNNER_ACCESS_CLIENT_ID", "CODEX_RUNNER_ACCESS_CLIENT_SECRET", "CODEX_RUNNER_SHARED_SECRET"]);
  assert.deepEqual(wrangler.vars, { ARTIFACT_REPOSITORY: "github", GITHUB_ARTIFACT_REPOSITORY_OWNER: "zailghe3", GITHUB_ARTIFACT_REPOSITORY_NAME: "fpo-artifacts", CODEX_RUNNER_BASE_URL: "https://codex-runner.pouchet.net" });
  assert.ok(Array.isArray(wrangler.kv_namespaces));
  assert.equal(wrangler.kv_namespaces.length, 1);
  const [catalogueCache] = wrangler.kv_namespaces;
  assert.equal(catalogueCache.binding, "ARTIFACT_CATALOGUE_CACHE");
  assert.equal(typeof catalogueCache.id, "string");
  assert.ok(catalogueCache.id.trim().length > 0);
});
