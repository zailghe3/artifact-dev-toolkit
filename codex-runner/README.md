...
`internal: true` disables external routing for an overlay. It is unrelated to Compose `external: true`, which says that the network lifecycle is operator-owned. Each proxy's uplink is its only non-internal network and therefore its only eligible external route; the policy smoke additionally assigns that test uplink explicit gateway priority so local Docker attachment order cannot select the role network. The controller alias `codex-runner` lets an existing tunnel origin such as `codex-runner:8789` continue to resolve after migration. Executor and proxy ports are not published.
...
## Managed publication isolation (revision 13)
