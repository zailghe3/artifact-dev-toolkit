...
When the managed Git push command fails, Repository Manager logs `repository_git_push_failed` with only a bounded failure class and an optional recognized upstream HTTP status; it never logs Git output, command arguments, URLs, or credentials.
