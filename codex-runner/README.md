...
Managed executor hosts must support the unprivileged Linux user namespaces required by pinned Codex's bubblewrap permission sandbox. As a deployment prerequisite, verify `kernel.unprivileged_userns_clone=1` and, when the corresponding sysctls exist, `kernel.apparmor_restrict_unprivileged_userns=0`; required GitHub-hosted verification prepares and checks these values before the image smoke. This enables the inner filesystem sandbox only. It grants no GitHub authority and does not replace the executor egress allowlist, read-only container root, `adt-managed` workspace policy, nested `.git` audit, or Repository Manager-only publication boundary.
...
Inspection of pinned Codex 0.153.4 source commit `3d2ee51ca2d5db578f328aa75e20aa22c0197c9a` confirms that legacy `workspace-write` starts with `:root = read`; it does not protect authentication state. Managed executor turns therefore never map `workspace-write` to `danger-full-access` and do not use that legacy read policy. Pinned Codex 0.153.4 uses the `adt-managed` permissions profile: minimal platform files are readable, the managed workspace is writable, all other filesystem roots are denied, and command network access is disabled. This is enforced by Codex’s bubblewrap split filesystem policy, not by hiding environment variables. The image smoke places non-secret sentinels at direct Codex-home, XDG/Git, and Runner-only paths and proves the real pinned sandbox cannot read them while it can edit the workspace. MCP servers, Apps, remote control, and hosted/cached/standalone web search are disabled by immutable App Server overrides. Shell commands additionally receive an empty home/XDG view, disabled system Git configuration, and no token/secret environment variables. A bounded non-symlink-following pre/post audit rejects `.git` metadata anywhere below the managed workspace.
