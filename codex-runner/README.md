...
The shared-secret-protected `GET /v1/diagnostics/auth-environment` endpoint performs only bounded, non-mutating checks against the fixed authentication host: DNS lookup, separate IPv4/IPv6 TCP port 443 and verified TLS handshakes, a safe `HEAD /` auth-origin/WAF check, custom-CA readability, proxy-presence flags, and `CODEX_HOME` permissions. It returns no addresses, paths, environment values, socket errors, or credentials, and never starts or polls device authentication. The TLS result is diagnostic only: Codex 0.118.0 uses its own Rust/reqwest trust and address-selection stack, so a successful Node TLS handshake does not prove that Codex authentication will succeed.
