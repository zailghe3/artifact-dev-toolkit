...
# Exercise the pinned Codex binary with the split executor's real security model:
# full access inside a non-root, capability-free, read-only-root container.
smoke_phase split-container-boundary
docker run --rm --read-only --cap-drop ALL \
  --tmpfs /tmp:size=268435456,mode=1777 --tmpfs /run:size=16777216,mode=0755 \
  --tmpfs /workspaces:uid=1000,gid=1000,mode=0700 \
  --mount "type=bind,src=$script_dir/split-boundary-probe.sh,dst=/run-split-boundary-probe,readonly" \
  "$image" /run-split-boundary-probe \
  || { echo "split_container_boundary_failed" >&2; exit 1; }
...
smoke_phase repository-manager
