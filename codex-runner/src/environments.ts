...
export async function loadRunnerEnvironments(file:string|undefined,probeFilesystem=true):Promise<RunnerEnvironment[]>{
 ...
 for(const raw of (parsed as {environments:unknown[]}).environments){
  ...
  keys.add(key);let canonical=cwd;if(probeFilesystem&&!managed)try{canonical=await realpath(cwd);if(!(await stat(canonical)).isDirectory())throw new Error()}catch{throw new Error("invalid_environments_configuration")}
  result.push({key,name:name.trim(),cwd:canonical,enabled,sandbox,ready:enabled&&!managed&&probeFilesystem?await workspaceReadiness(canonical,sandbox):false,...(managed?{repository:managed}:{})});
 }
