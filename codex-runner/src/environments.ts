...
export interface ManagedRepositoryConfiguration{managed:true;owner:string;repo:string;baseBranch:string}
export interface RunnerEnvironment{key:string;name:string;cwd:string;enabled:boolean;sandbox:RunnerSandbox;ready:boolean;repository?:ManagedRepositoryConfiguration}
export interface PublicRunnerEnvironment{key:string;name:string;enabled:boolean;ready:boolean;sandbox:RunnerSandbox;managedRepository?:boolean}
...
export async function loadRunnerEnvironments(file:string|undefined,probeFilesystem=true):Promise<RunnerEnvironment[]>{
 ...
 for(const raw of (parsed as {environments:unknown[]}).environments){
  if(!raw||typeof raw!=="object"||Array.isArray(raw)||Object.keys(raw as object).some(k=>!["key","name","cwd","enabled","sandbox","repository"].includes(k))||!["key","name","cwd","enabled","sandbox"].every(k=>Object.hasOwn(raw as object,k)))throw new Error("invalid_environments_configuration");
  const {key,name,cwd,enabled,sandbox,repository}=raw as Record<string,unknown>;
  ...
  let managed:ManagedRepositoryConfiguration|undefined;if(repository!==undefined){if(sandbox!=="workspace-write"||!repository||typeof repository!=="object"||Array.isArray(repository)||!exact(repository as Record<string,unknown>,["managed","owner","repo","baseBranch"]))throw new Error("invalid_environments_configuration");const r=repository as Record<string,unknown>;if(r.managed!==true||typeof r.owner!=="string"||!/^[A-Za-z0-9](?:[A-Za-z0-9-]{0,38})$/.test(r.owner)||typeof r.repo!=="string"||!/^[A-Za-z0-9._-]{1,100}$/.test(r.repo)||r.repo==="."||r.repo===".."||typeof r.baseBranch!=="string"||!/^[A-Za-z0-9](?:[A-Za-z0-9._/-]{0,98}[A-Za-z0-9])?$/.test(r.baseBranch)||r.baseBranch.includes("..")||r.baseBranch.includes("//")||r.baseBranch.startsWith("adt/codex/"))throw new Error("invalid_environments_configuration");managed={managed:true,owner:r.owner,repo:r.repo,baseBranch:r.baseBranch}}
  keys.add(key);let canonical=cwd;if(probeFilesystem)try{if(managed){await mkdir(cwd,{recursive:true,mode:0o700});canonical=await realpath(cwd)}else canonical=await realpath(cwd);if(!(await stat(canonical)).isDirectory())throw new Error()}catch{throw new Error("invalid_environments_configuration")}
  result.push({key,name:name.trim(),cwd:canonical,enabled,sandbox,ready:enabled?(managed?true:probeFilesystem?await workspaceReadiness(canonical,sandbox):false):false,...(managed?{repository:managed}:{})});
 }
 return result.sort((a,b)=>a.name.localeCompare(b.name)||a.key.localeCompare(b.key));
}
export async function workspaceReadiness(cwd:string,sandbox:RunnerSandbox){try{await access(cwd,constants.R_OK|constants.X_OK);if(sandbox==="workspace-write"){const probe=join(cwd,`.adt-runner-probe-${randomBytes(12).toString("hex")}`);try{const handle=await open(probe,"wx",0o600);await handle.close()}finally{await rm(probe,{force:true})}}return true}catch{return false}}
export function publicEnvironment(value:RunnerEnvironment):PublicRunnerEnvironment{return{key:value.key,name:value.name,enabled:value.enabled,ready:value.ready,sandbox:value.sandbox,...(value.repository?{managedRepository:true}:{})}}
