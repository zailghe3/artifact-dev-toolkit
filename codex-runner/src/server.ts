...
export function safeAuthDiagnosticsLog(value:AuthEnvironmentDiagnostics){const log:Record<string,string|number|boolean>={level:"info",event:"auth_transport_diagnostics",rustProbeStatus:value.codexAuthTransportProbeStatus};const addTransport=(prefix:string,probe:TransportProbeResult|undefined)=>{if(!probe)return;log[`${prefix}Outcome`]=probe.outcome;if(probe.outcome==="http_response")log[`${prefix}Status`]=probe.status;else for(const key of ["isTimeout","isConnect","isRequest","ioErrorKind","tlsSourcePresent","sourceClass"] as const)log[`${prefix}${key.charAt(0).toUpperCase()}${key.slice(1)}`]=probe[key]};addTransport("rustNative",value.codexAuthTransportProbe?.baseline);addTransport("rustRustls",value.codexAuthTransportProbe?.rustlsTls);if(value.nodeDeviceAuthPostProbe?.status!==undefined)log.nodeMalformedStatus=value.nodeDeviceAuthPostProbe.status;return log}
