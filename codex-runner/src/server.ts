...
export function safeAuthDiagnosticsLog(value:AuthEnvironmentDiagnostics){return{level:"info",event:"auth_environment_diagnostics",codexReady:value.codexAppServerReady,systemCaAvailable:value.systemCaBundlePresent&&value.systemCaBundleReadable&&value.systemCaBundleNonEmpty,ipv4Tls:value.ipv4TlsConnectivity,ipv6Tls:value.ipv6TlsConnectivity,proxyConfigured:value.httpProxyConfigured||value.httpsProxyConfigured||value.allProxyConfigured}}
...
async function boundedBody(request:IncomingMessage,limit:number){let size=0;for await(const chunk of request){size+=Buffer.byteLength(chunk);if(size>limit)throw new SafeError("request_too_large",413)}if(size>0&&!request.headers["content-type"]?.startsWith("application/json"))throw new SafeError("json_content_type_required",415);return size}
export function createRunnerServer(configuration:RunnerConfiguration,appServer:AppServerClient=new StdioAppServerClient(configuration.codexCommand,configuration.runnerVersion),diagnostics:()=>Promise<AuthEnvironmentDiagnostics>=()=>diagnoseAuthEnvironment({codexCommand:configuration.codexCommand,codexAppServerReady:()=>appServer.readiness()})){
 const auth=new AuthenticationService(appServer);return createServer({requestTimeout:15_000,headersTimeout:10_000},async(request,response)=>{const path=new URL(request.url??"/","http://runner.invalid").pathname;try{
  ...
  const bodySize=await boundedBody(request,configuration.requestBodyLimit);
  ...
  if(request.method==="POST"&&path==="/v1/codex/test"){if(bodySize!==0)throw new SafeError("invalid_request",400);if(!appServer.testCodex)return json(response,503,{ok:false,reason:"app_server_unavailable"});return json(response,200,await appServer.testCodex())}
