...
export interface RunnerConfiguration { host:string; port:number; sharedSecret:string; runnerVersion:string; codexCommand:string; requestBodyLimit:number; jobRequestBodyLimit:number; deviceAuthCompatible:boolean;environmentsFile?:string;stateDirectory:string }
...
export async function loadConfiguration(environment:NodeJS.ProcessEnv=process.env):Promise<RunnerConfiguration>{
  ...
  return{host:environment.HOST??"0.0.0.0",port,sharedSecret:await loadSecret(environment),runnerVersion:environment.CODEX_RUNNER_VERSION??"development",codexCommand:environment.CODEX_COMMAND??"codex",requestBodyLimit:16_384,jobRequestBodyLimit:393_216,deviceAuthCompatible:environment.CODEX_DEVICE_AUTH_COMPATIBLE==="1",environmentsFile:environment.CODEX_RUNNER_ENVIRONMENTS_FILE,stateDirectory:environment.CODEX_RUNNER_STATE_DIR??"/data/runner"};
}
