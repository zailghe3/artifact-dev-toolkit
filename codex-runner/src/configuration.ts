...
export type RunnerRole="integrated"|"controller"|"executor"|"repository-manager";
export interface RunnerConfiguration { role?:RunnerRole;host:string; port:number; sharedSecret?:string;executorSigningPrivateKey?:string;executorVerifyingPublicKey?:string;executorBaseUrl?:string;repositoryManagerBaseUrl?:string;repositoryStateRoot?:string;workspaceRoot?:string;redeployWebhook?:string;runnerVersion:string; codexCommand:string; requestBodyLimit:number; jobRequestBodyLimit:number; deviceAuthCompatible:boolean;environmentsFile?:string;stateDirectory:string;jobDurationMs:number;codexHome:string;sqliteHome?:string;sqliteBackupRoot:string;sqliteBackupIntervalMs:number;sqliteBackupRetention:number }
...
export async function loadConfiguration(environment:NodeJS.ProcessEnv=process.env):Promise<RunnerConfiguration>{
  ...
  const role=(environment.CODEX_RUNNER_ROLE??"integrated") as RunnerRole;if(!["integrated","controller","executor","repository-manager"].includes(role))throw new Error("invalid_runner_role");
  ...
  const sharedSecret=role==="executor"||role==="repository-manager"?undefined:await loadSecret(environment);
  ...
  const executorVerifyingPublicKey=role==="executor"||role==="repository-manager"?await readNamed("CODEX_RUNNER_EXECUTOR_VERIFYING_PUBLIC_KEY","CODEX_RUNNER_EXECUTOR_VERIFYING_PUBLIC_KEY_FILE","executor_verifying_public_key"):undefined;
  ...
  const repositoryManagerBaseUrl=role==="controller"?(environment.CODEX_RUNNER_REPOSITORY_MANAGER_URL??"http://codex-runner-repository-manager:8791"):undefined;if(repositoryManagerBaseUrl){const url=new URL(repositoryManagerBaseUrl);if(url.protocol!=="http:"||url.username||url.password||url.pathname!=="/"||url.search||url.hash)throw new Error("invalid_repository_manager_url")}
  ...
  const codexHome=environment.CODEX_HOME??"/data/codex",sqliteHome=environment.CODEX_SQLITE_HOME?.trim()||undefined,sqliteBackupRoot=environment.CODEX_RUNNER_SQLITE_BACKUP_ROOT??`${codexHome}/.adt/sqlite-backups`,sqliteBackupIntervalMs=Number(environment.CODEX_RUNNER_SQLITE_BACKUP_INTERVAL_MS??"86400000"),sqliteBackupRetention=Number(environment.CODEX_RUNNER_SQLITE_BACKUP_RETENTION??"14");
  if(![codexHome,sqliteHome,sqliteBackupRoot].filter(Boolean).every(value=>value!.startsWith("/")))throw new Error("sqlite_storage_path_must_be_absolute");
  if(!Number.isSafeInteger(sqliteBackupIntervalMs)||sqliteBackupIntervalMs<60_000||sqliteBackupIntervalMs>31*86_400_000)throw new Error("invalid_sqlite_backup_interval");
  if(!Number.isSafeInteger(sqliteBackupRetention)||sqliteBackupRetention<1||sqliteBackupRetention>365)throw new Error("invalid_sqlite_backup_retention");
  const repositoryStateRoot=environment.CODEX_RUNNER_REPOSITORY_STATE_ROOT??"/data/repositories";if(!repositoryStateRoot.startsWith("/"))throw new Error("repository_state_path_must_be_absolute");
  return{role,host:environment.HOST??"0.0.0.0",port,sharedSecret,executorSigningPrivateKey,executorVerifyingPublicKey,executorBaseUrl,repositoryManagerBaseUrl,repositoryStateRoot,workspaceRoot:environment.CODEX_RUNNER_WORKSPACE_ROOT??"/workspaces",redeployWebhook,runnerVersion:environment.CODEX_RUNNER_VERSION??"development",codexCommand:environment.CODEX_COMMAND??"codex",requestBodyLimit:16_384,jobRequestBodyLimit:JOB_REQUEST_BODY_LIMIT,deviceAuthCompatible:environment.CODEX_DEVICE_AUTH_COMPATIBLE==="1",environmentsFile:environment.CODEX_RUNNER_ENVIRONMENTS_FILE,stateDirectory:environment.CODEX_RUNNER_STATE_DIR??"/data/runner",jobDurationMs,codexHome,sqliteHome,sqliteBackupRoot,sqliteBackupIntervalMs,sqliteBackupRetention};
}
