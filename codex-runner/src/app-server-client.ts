...
export type WorkflowTurnResult={ok:true;outputText:string;threadId:string;turnId:string}|{ok:false;reason:"thread_start_failed"|"turn_start_failed"|"turn_failed"|"interaction_required"|"output_too_large";threadId?:string;turnId?:string};
export interface AppServerClient {readiness():Promise<boolean>;status():Promise<unknown>;startDeviceLogin():Promise<unknown>;logout():Promise<unknown>;testCodex?():Promise<CodexTestResult>;listModels?():Promise<SafeCodexModel[]>;runWorkflowTurn?(input:{cwd:string;sandbox:"read-only"|"workspace-write";prompt:string;model?:string;reasoningEffort?:string;onStarted?:(threadId:string,turnId:string)=>void}):Promise<WorkflowTurnResult>;interruptWorkflowTurn?(threadId:string,turnId:string):Promise<void>;close():Promise<void>}
...
export class StdioAppServerClient implements AppServerClient{
  ...
  async testCodex():Promise<CodexTestResult>{
    ...
    try{
      ...
      let thread:unknown;try{thread=await this.healthRequest("thread/start",{cwd,approvalPolicy:"never",sandbox:"read-only",ephemeral:true},operationDeadline)}catch(error){return{ok:false,reason:error instanceof HealthDeadlineError?"timeout":"thread_start_failed"}}
  ...
  async listModels():Promise<SafeCodexModel[]>{
    ...
    for(let page=0;page<10&&output.length<100;page++){const value=await this.afterReady("model/list",{includeHidden:false,limit:25,...(cursor?{cursor}:{})}) as Record<string,unknown>,data=value.data;if(!Array.isArray(data))throw new Error("invalid_model_catalog");for(const raw of data){if(!raw||typeof raw!=="object"||Array.isArray(raw))throw new Error("invalid_model_catalog");const model=raw as Record<string,unknown>,efforts=model.supportedReasoningEfforts;if(typeof model.id!=="string"||!model.id||model.id.length>120||typeof model.model!=="string"||!model.model||model.model.length>120||typeof model.displayName!=="string"||!model.displayName||model.displayName.length>120||typeof model.isDefault!=="boolean"||typeof model.defaultReasoningEffort!=="string"||model.defaultReasoningEffort.length>40||!Array.isArray(efforts))throw new Error("invalid_model_catalog");const safeEfforts=efforts.map(item=>{if(!item||typeof item!=="object"||Array.isArray(item)||typeof (item as Record<string,unknown>).reasoningEffort!=="string"||typeof (item as Record<string,unknown>).description!=="string")throw new Error("invalid_model_catalog");return{reasoningEffort:String((item as Record<string,unknown>).reasoningEffort).slice(0,40),description:String((item as Record<string,unknown>).description).slice(0,240)}});output.push({id:model.id,threadModel:model.model,displayName:model.displayName,isDefault:model.isDefault,defaultReasoningEffort:model.defaultReasoningEffort,supportedReasoningEfforts:safeEfforts});if(output.length>=100)break}cursor=typeof value.nextCursor==="string"&&value.nextCursor.length<=200?value.nextCursor:undefined;if(!cursor)break}return output;
  }
  async runWorkflowTurn(input:{cwd:string;sandbox:"read-only"|"workspace-write";prompt:string;model?:string;reasoningEffort?:string;onStarted?:(threadId:string,turnId:string)=>void}):Promise<WorkflowTurnResult>{
  ...
  private async healthRequest(method:string,params:unknown,deadline:number){await this.within(this.ready(),deadline);const remaining=this.remaining(deadline);if(remaining<=0)throw new HealthDeadlineError();try{return await this.within(this.request(method,params,Math.min(this.timeoutMs,remaining)),deadline)}catch(error){if(this.remaining(deadline)<=0)throw new HealthDeadlineError();throw error}}
