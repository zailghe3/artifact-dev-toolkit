...
export class StdioAppServerClient implements AppServerClient{
  ...
  async closeAndWait(){const process=this.process;if(!process)return;const exited=new Promise<void>(resolve=>{process.once("exit",()=>resolve());process.once("close",()=>resolve())});this.failProcess();let timer:NodeJS.Timeout|undefined;const graceful=await Promise.race([exited.then(()=>true),new Promise<false>(resolve=>{timer=setTimeout(()=>resolve(false),APP_SERVER_SHUTDOWN_MS);timer.unref()})]);if(timer)clearTimeout(timer);if(graceful)return;if(!process.killed||process.exitCode===null)process.kill("SIGKILL");await Promise.race([exited,new Promise<void>(resolve=>{const fallback=setTimeout(resolve,1_000);fallback.unref()})])}
