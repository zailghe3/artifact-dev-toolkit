...
export type AppServerStartupCategory="ready"|"spawn_failed"|"process_exited"|"initialize_timeout"|"initialize_rpc_error"|"stdin_failure"|"stdout_closed"|"unknown";
...
export const APP_SERVER_STDERR_MAX_LINES=12,APP_SERVER_STDERR_MAX_LINE=512,APP_SERVER_SHUTDOWN_MS=5_000;
export function sanitizeAppServerStderr(value:string){return value.replace(/\x1B(?:[@-_][0-?]*[ -/]*[@-~]|\][^\x07]*(?:\x07|\x1B\\))/g,"").replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F-\x9F]/g,"").replace(/(https?:\/\/)[^\s/@:]+:[^\s/@]+@/ig,"$1[REDACTED]@").replace(/\bbearer\s+[^\s,;"']+/ig,"Bearer [REDACTED]").replace(/(["']?(?:api[_-]?key|apikey|access[_-]?token|refresh[_-]?token|token|secret|password|credential)s?["']?\s*[:=]\s*)["']?[^\s,;}"']+["']?/ig,"$1[REDACTED]").slice(0,APP_SERVER_STDERR_MAX_LINE)}
export function preserveStartupDiagnostic(current:AppServerStartupDiagnostic,category:AppServerStartupCategory,elapsedMs:number,details:Partial<AppServerStartupDiagnostic>={}){return current.category==="unknown"?{...current,...details,category,elapsedMs:Math.max(0,elapsedMs)}:current}
...
const EXECUTOR_SHELL_NAMES=["PATH","HOME","LANG","LC_ALL","TERM","HTTP_PROXY","HTTPS_PROXY","ALL_PROXY","NO_PROXY"] as const;
export function executorShellEnvironment(environment:NodeJS.ProcessEnv=process.env){const safe:Record<string,string>={};for(const name of EXECUTOR_SHELL_NAMES){const value=environment[name]??environment[name.toLowerCase()];if(!value||value.length>2048)continue;if(name.endsWith("PROXY")&&name!=="NO_PROXY"){try{const url=new URL(value);if(url.protocol!=="http:"||url.username||url.password)continue}catch{continue}}safe[name]=value}return safe}
export function executorCodexConfigOverrides(environment:NodeJS.ProcessEnv=process.env){const safe=executorShellEnvironment(environment),set=Object.entries(safe).map(([key,value])=>`${JSON.stringify(key)}=${JSON.stringify(value)}`).join(","),sqliteHome=environment.CODEX_SQLITE_HOME?.trim();return[...(sqliteHome?[`sqlite_home=${JSON.stringify(sqliteHome)}`]:[]),"allow_login_shell=false",'web_search="disabled"','shell_environment_policy.inherit="none"',`shell_environment_policy.set={${set}}`,'shell_environment_policy.exclude=["*KEY*","*SECRET*","*TOKEN*","*PASSWORD*","*CREDENTIAL*","*AUTH*"]']}
export interface AppServerClient {readiness():Promise<boolean>;status():Promise<unknown>;startDeviceLogin():Promise<unknown>;logout():Promise<unknown>;testCodex?(sandbox?:"read-only"|"danger-full-access"):Promise<CodexTestResult>;listModels?():Promise<SafeCodexModel[]>;runWorkflowTurn?(input:{cwd:string;sandbox:"read-only"|"workspace-write"|"danger-full-access";prompt:string;model?:string;reasoningEffort?:string;onStarted?:(threadId:string,turnId:string)=>void;onActivity?:(category:"model"|"tool"|"interaction")=>void}):Promise<WorkflowTurnResult>;observeWorkflowTurn?(generation:string,executionId:string):Promise<WorkflowTurnResult>;reconcileUnknownExecution?():Promise<WorkflowTurnResult>;interruptWorkflowTurn?(threadId:string,turnId:string):Promise<void>;closeAndWait?():Promise<void>;close():Promise<void>}
...
export class StdioAppServerClient implements AppServerClient{
  ...
  private recordStartup(category:AppServerStartupCategory,details:Partial<AppServerStartupDiagnostic>={}){this.startup=preserveStartupDiagnostic(this.startup,category,Date.now()-this.startupAt,details)}
  ...
  private start(){
    ...
    child.stderr.setEncoding("utf8");child.stderr.on("data",chunk=>{for(const line of String(chunk).split(/\r?\n/)){if(!line)continue;this.startup.stderrTail.push(sanitizeAppServerStderr(line));if(this.startup.stderrTail.length>APP_SERVER_STDERR_MAX_LINES)this.startup.stderrTail.shift()}});
    child.once("error",()=>{this.recordStartup("spawn_failed");this.failProcess()});
    child.once("exit",(code,signal)=>{this.recordStartup("process_exited",{...(code===null?{}:{exitCode:code}),...(signal?{signal}:{})});this.failProcess()});
    child.stdin.once("error",()=>{this.recordStartup("stdin_failure");this.failProcess()});
    child.stdout.once("close",()=>this.recordStartup("stdout_closed"));
  }
  private async initialize(){try{await this.request("initialize",{clientInfo:{name:"adt_codex_runner",title:"ADT Codex Runner",version:this.runnerVersion}});await this.write({method:"initialized"});this.recordStartup("ready");}catch(error){this.recordStartup(error instanceof RequestTimeoutError?"initialize_timeout":"initialize_rpc_error");this.failProcess();throw new Error("app_server_initialization_failed")}}
  ...
  async closeAndWait(){const process=this.process;if(!process)return;let observed=false;const exited=new Promise<void>(resolve=>{const done=()=>{observed=true;resolve()};process.once("exit",done);process.once("close",done)});const wait=async(ms:number)=>{let timer:NodeJS.Timeout|undefined;const result=await Promise.race([exited.then(()=>true),new Promise<false>(resolve=>{timer=setTimeout(()=>resolve(false),ms);timer.unref()})]);if(timer)clearTimeout(timer);return result};this.failProcess();if(await wait(this.shutdownGraceMs))return;process.kill("SIGKILL");if(await wait(this.shutdownKillMs))return;if(!observed)throw new Error("app_server_shutdown_unconfirmed")}
  async close(){await this.closeAndWait()}
}
