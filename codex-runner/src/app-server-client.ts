...
export function executorCodexConfigOverrides(environment:NodeJS.ProcessEnv=process.env){const safe=executorShellEnvironment(environment),set=Object.entries(safe).map(([key,value])=>`${JSON.stringify(key)}=${JSON.stringify(value)}`).join(",");return["allow_login_shell=false",'web_search="disabled"','shell_environment_policy.inherit="none"',`shell_environment_policy.set={${set}}`,'shell_environment_policy.exclude=["*KEY*","*SECRET*","*TOKEN*","*PASSWORD*","*CREDENTIAL*","*AUTH*"]']}
export interface AppServerClient {readiness():Promise<boolean>;status():Promise<unknown>;startDeviceLogin():Promise<unknown>;logout():Promise<unknown>;testCodex?(sandbox?:"read-only"|"danger-full-access"):Promise<CodexTestResult>;listModels?():Promise<SafeCodexModel[]>;runWorkflowTurn?(input:{cwd:string;sandbox:"read-only"|"workspace-write"|"danger-full-access";prompt:string;model?:string;reasoningEffort?:string;onStarted?:(threadId:string,turnId:string)=>void;onActivity?:(category:"model"|"tool"|"interaction")=>void}):Promise<WorkflowTurnResult>;observeWorkflowTurn?(generation:string,executionId:string):Promise<WorkflowTurnResult>;reconcileUnknownExecution?():Promise<WorkflowTurnResult>;interruptWorkflowTurn?(threadId:string,turnId:string):Promise<void>;close():Promise<void>}
