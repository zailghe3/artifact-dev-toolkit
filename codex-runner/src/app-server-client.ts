...
export function sanitizeAppServerStderr(value:string){return value.replace(/(https?:\/\/)[^\s/@:]+:[^\s/@]+@/ig,"$1[REDACTED]@").replace(/\bbearer\s+[^\s,;"']+/ig,"Bearer [REDACTED]").replace(/(["']?(?:api[_-]?key|apikey|access[_-]?token|refresh[_-]?token|token|secret|password|credential)s?["']?\s*[:=]\s*)["']?[^\s,;}"']+["']?/ig,"$1[REDACTED]").slice(0,APP_SERVER_STDERR_MAX_LINE)}
