...
export class EmergencyStopController{
 ...
 async stop(generation:string|undefined,interrupt:()=>Promise<void>){await this.persist({emergencyStopped:true,...(generation?{stoppedGeneration:generation}:{}),updatedAt:new Date().toISOString(),hardRestart:{attempted:false,succeeded:false}});await interrupt().catch(()=>{});let hardRestart:EmergencyState["hardRestart"]={attempted:false,succeeded:false,reason:"not_configured"};if(this.webhook){try{const initial=new URL(this.webhook),response=await fetch(initial,{method:"POST",redirect:"manual",signal:AbortSignal.timeout(10_000)});hardRestart={attempted:true,succeeded:response.status>=200&&response.status<300};if(!hardRestart.succeeded)hardRestart.reason="request_failed"}catch{hardRestart={attempted:true,succeeded:false,reason:"request_failed"}}}await this.persist({...this.state,hardRestart,updatedAt:new Date().toISOString()});return this.snapshot()}
