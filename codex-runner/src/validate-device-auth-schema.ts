...
export function validateCodexTestSchemas(input:unknown[]){
 ...
 if(!turnProps||!required(turn,"threadId")||!hasType(turnProps.threadId,"string")||!required(turn,"input")||!inputSchema||!hasType(inputSchema,"array")||!textVariant||!required(textVariant,"text")||!textProps||!hasType(textProps["text"],"string")||!hasType(resolveSchema(client,turnProps.effort),"string"))throw new Error("codex_test_schema_missing_turn_contract");
 const modelList=routed(client,"model/list"),modelListProps=properties(modelList);
 if(!modelListProps||!hasType(modelListProps.includeHidden,"boolean")||!hasType(modelListProps.limit,"integer")||!hasType(modelListProps.cursor,"string"))throw new Error("codex_test_schema_missing_model_list_request");
 ...
 if(!responseProps||!required(modelResponse.schema,"data")||!data||!hasType(data,"array")||!model||!modelProps||!["id","model","displayName","isDefault","defaultReasoningEffort","supportedReasoningEfforts"].every(field=>required(model,field))||!["id","model","displayName","defaultReasoningEffort"].every(field=>hasType(modelProps[field],"string"))||!hasType(resolveSchema(modelResponse,modelProps.isDefault),"boolean")||!efforts||!hasType(efforts,"array")||!effort||!effortProps||!required(effort,"reasoningEffort")||!required(effort,"description")||!hasType(resolveSchema(modelResponse,effortProps.reasoningEffort),"string")||!hasType(resolveSchema(modelResponse,effortProps.description),"string")||!hasType(resolveSchema(modelResponse,responseProps.nextCursor),"string"))throw new Error("codex_test_schema_missing_model_list_response");
